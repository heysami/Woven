...
**Named-reference research gate (BEFORE the Phase A lock).** When the committed direction is (or contains) a named real-world reference - a real game / film / brand / product / place ("i want mobile legends", "OutRun style") - the reference IS the vision, and you may not lock ANY interpretive rider about it (dimensionality, camera, materials, register, era) from training-data recall. WebSearch/WebFetch how the real thing ACTUALLY presents first, and derive the ledger detail from that research. Recall is routinely outdated or plain wrong, and whatever you write into the decision ledger here is treated as the user's committed vision by every downstream orchestrator and drawer - a recalled "2.5D isometric" rider on a full-3D game corrupts the entire build. When the user typed the reference freeform (no gate click), the researched characterization goes into the lock message you show them, so a wrong read is visible immediately.
