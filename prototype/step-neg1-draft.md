...
3. Generate **1 frame by default, 2 frames maximum** (one hero, one optional secondary view) at small thumbnail size (≤768px longest side). The default is ONE — the user said "give me a quick preview", not "render the whole site". Save under `.prototype-options/<TURN_SLUG>/draft-<k>.png` using the same TURN_SLUG as the preview recolours.
