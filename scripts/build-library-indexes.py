...
prototype/<prefix>-*.md (these are the source of truth).
...
Per-entry files are at prototype/<prefix>-<entryId>.md with YAML frontmatter.
This script ONLY reads those — no big library file is consulted.

Run after editing any prototype/<prefix>-*.md file:
...
PROTOTYPE_DIR = os.path.join(ROOT, "prototype")
...
def build_index(lib):
    ...
    files = sorted(f for f in os.listdir(PROTOTYPE_DIR) if f.startswith(prefix + "-") and f.endswith(".md"))
    ...
    for fname in files:
        ...
        entry["sourceFile"] = f"prototype/{fname}"
...
def main():
    for lib in LIBS:
        build_index(lib)
    print(f"\nIndexes regenerated by scanning {os.path.relpath(PROTOTYPE_DIR, ROOT)}/ — per-entry files are the source of truth.")
