...
- `input-gamepad` [] - (o) leftStick(vector2) rightStick(vector2) x(number) y(number) a(boolean) b(boolean) connected(boolean). First connected gamepad; no permission.
- `input-accel` [] - (o) x(number) y(number) z(number) ready(boolean). Device motion / accelerometer; permission-gated (mobile).
- `input-midi` [] - (o) note(number) velocity(number) cc(number) ccValue(number) gate(boolean) ready(boolean). WebMIDI controller; permission-gated; velocity/ccValue normalized 0..1.
...
DATA (async web data; the fetch is fire-and-forget in node state, never blocks the engine; cross-origin needs a CORS endpoint or the /__web_proxy):
- `dat-fetch` [url, method: GET|POST, pollMs] - (o) text(string) value(number) ok(boolean) updated(event). Poll a URL; pollMs=0 fetches once; `updated` pulses on each new response. Wire `text` into op-json-path to pull a field.
