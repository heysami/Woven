...
_VECTOR_AUTHORING = (
    ...
    "      • star    - a `path` that ALSO carries a `star` param block, so the editor's spike / depth / "
    "roundness sliders keep working on it: {\"type\":\"path\",\"closed\":true,\"d\":\"<generated>\","
    "\"star\":{\"cx\":,\"cy\":,\"rx\":,\"ry\":,\"points\":<3..60 spikes>,\"innerRatio\":<0.05..1 valley radius as a "
    "fraction of the outer radius; 1 = a regular polygon>,\"roundness\":<0..1 fillet on every tip AND valley>}}. "
    "Emit BOTH the params and a matching `d`; the editor regenerates `d` from the params on the next edit. "
    "Use this for stars, bursts, flowers, gears and regular polygons instead of hand-writing the path.\n"
...
_HYPERFRAMES_AUTHORING = (
    ...
    "\"align\":\"left|center|right\",\"ease\":\"power3.out\",\n"
    "         \"keys\":[{\"t\":0,\"x\":-80,\"y\":0,\"opacity\":0,\"scale\":0.94,\"rotation\":0},"
    "{\"t\":0.9,\"x\":0,\"y\":0,\"opacity\":1,\"scale\":1,\"rotation\":0,\"ease\":\"power3.out\"},"
    "{\"t\":2.4,\"x\":0,\"y\":-24,\"opacity\":1,\"scale\":1.04,\"rotation\":0,\"ease\":\"sine.inOut\"}]}]}\n"
    "  MOTION IS KEYFRAMED. `keys` is the animation: a list of poses at times `t` measured "
    "from the CLIP's own start (0 <= t <= the clip's duration), sorted ascending. Each key's "
    "`ease` governs the segment ARRIVING at it, so the first key needs none - it is just the "
    "pose the clip opens in. Use as many keys as the motion needs (a hold, a settle, an exit); "
    "two keys is only the simplest case, and one key pins a static pose for the clip's length. "
    "The clip-level `ease` is the fallback for keys that omit one.\n"
    "  LEGACY `from`/`to`: a clip carrying only `from`+`to` and no `keys` still loads (it "
    "upgrades to a two-key list), and the editor keeps `from`/`to` written back as read-only "
    "mirrors of the first + last key. Never hand-edit them to animate - they are ignored "
    "whenever `keys` is present. Author `keys`.\n"
