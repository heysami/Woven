...
| **ANY ONE section** is a cinematic PRESENTATION beat - full-bleed video/imagery + UI choreographed together, scrubbed or stepped, linear within itself. **Test this per SECTION, never against the whole brief** - the family claims individual surfaces and hands the rest back as `hygienicScope` (nav, footer, pricing, docs, forms), so "the app also has features" does NOT falsify it. **The register is ANYTHING THAT MOVES AND TELLS A STORY, not just commerce**: Apple-product-page and motionsites are the famous examples, but a museum or exhibition walk, a heritage or anniversary film, a documentary explainer, a scrolled timeline, a title sequence and a scientific fly-through are the same shape and are often the STRONGER case, because the whole point of the surface is the moving image. Do not reject it because the subject is cultural rather than commercial. **STRUCTURAL TRIGGER - the committed SHELL can settle this on its own**: `shell-scroll-journey-scene` (one continuous scene the scroll travels through, stations not sections) and `shell-horizontal-scroll-stage` (full-bleed chaptered panels on a wheel-driven X track) ARE this family's shape by construction - they are built from full-bleed media plus copy in a quiet zone, scrubbed by scroll. When the direction lock names one of those shells, motion-studio-orchestrator goes on the card CHECKED unless the user has said no; treating such a build as stills-only is the silent downgrade this row exists to stop | `motion-studio-orchestrator` - **`mode=brainstorm` BEFORE writing any HTML** |
...
3. Pre-tick the orchestrators you actually recommend with the bare `checked` attribute. Also list 1-3 plausible-but-not-recommended candidates UNchecked when they exist (with the plan line saying what opting in would add), so the user can opt in - N/A-by-default is a valid proposal shape. Always include a `none` option last. **A family you judged "probably not" belongs on the card UNCHECKED, not omitted** - omission silently makes the taste call the gate exists to hand the user. Two concrete consequences, both about TIME-BASED media, which is the lane that goes missing most:
   ...
   - **`motion-studio-orchestrator` is at minimum an unchecked option with its cost line** whenever a video provider shows `✓ KEY` and the brief has any surface whose job is to be WATCHED - hero, launch, brand, product, portfolio, and equally museum / exhibition / heritage / documentary / explainer / timeline / title-sequence work. It goes on CHECKED when the committed shell is `shell-scroll-journey-scene` or `shell-horizontal-scroll-stage`, which are that family's shape by construction (see the predicate table). A cultural or narrative subject is not a reason to drop it; those briefs are usually the ones that most want moving image.
