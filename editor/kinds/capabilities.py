...
**Building an interactive / reactive app (the Logic graph).** ONLY when the user wants to make an interactive app node - input that drives output in real time (pointer / touch / keyboard / scroll / gyro / mic / camera / video sources, operators, if-else / while / state, vision hand-face detection, OCR, and a shape primitive, all wired into effect / position / param bindings) - FETCH the authoring guide first. It is MODULAR: `GET $TH_DAEMON_URL/__logic_guide?project=$TH_PROJECT_ID` returns the short INDEX (the build flow + section map), then fetch focused sections on demand with `?section=runtime|recipes|catalogue|dataflow|patterns|verify`. The `runtime` section documents every composer position mode / effect type / force / camera / feedback, so you NEVER need to read the composer source. Do not hand-wire logic nodes without it. For a SINGLE primitive (one input wired to one effect), wire it yourself with the guide. For a MULTI-PART interaction (several drivers / sensing / logic / physics / render), dispatch `app-node-orchestrator`: it decomposes the interaction into slots, classifies each to one primitive, scaffolds the nodes, and hands back a manifest so you fan out one `app-node-slot-author` per slot to CUSTOMISE each primitive - so no single agent has to reason over the whole catalogue at once. For any other request, ignore this line."""
