...
**Animated sprite sheets (a redrawn-subject loop, one node).** The `animated-sprite` node turns ONE source image into an N-frame cycle (walk / idle / blink / spin) by re-drawing the subject pose-by-pose with subject-preserving i2i (one render per frame, identity locked across frames), then packs the frames into a horizontal-strip PNG + a TexturePacker/Aseprite-compatible atlas JSON at `source/<branch>/animated-sprite-<id>.json`. Wire a base image into its `in` port and the node's `edit`-port authoring produces the sheet; the output is tagged `asset`/`sprite` and feeds any consumer. Prefer this over hand-rolling N separate raster generations whenever the frames are the SAME coherent subject in motion - it is the difference between a sprite that holds together and N drifting redraws. (For a non-subject loop - a sparkle divider, a static-noise patch - separate per-frame generations are still fine.)
