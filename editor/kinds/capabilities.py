...
**Named-reference fidelity - NEVER characterize a real thing from model memory.** When the user names a real game / film / brand / product / place as the reference for what to build ("like Mobile Legends", "OutRun style", "a Notion-like"), that reference IS the vision, and fidelity to it means fidelity to how the real thing ACTUALLY presents. Before writing ANY characterization of a named reference into a decision, decision-ledger detail, option label, contract, or dispatch brief (dimensionality, camera, materials, layout register, palette, era), research the real thing THIS TURN: WebSearch/WebFetch its actual presentation, or read refs the user supplied. Training-data recall about a named reference is banned as the sole source: it is routinely outdated or plain wrong, and a wrong rider written here gets treated as the user's committed vision by every downstream agent (canonical failure: a chat recalled "Mobile Legends is a 2.5D isometric sprite MOBA" and ledger-locked `camera: 2.5d`; the real game is full 3D under a fixed high-angle camera, and one search would have caught it). If you cannot research it this turn, say so and ask; never fill the gap silently from recall.
