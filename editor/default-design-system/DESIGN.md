...
| Rams | `data-theme="rams"` | Elegant Dieter Rams / Braun skeuomorphism: a warm light-grey device face (NOT white), depth read from a single overhead light - every surface carries a very subtle top-to-bottom gradient + a hairline bevel (1px highlight on the upper lip, 1px warm shadow on the lower lip). Signature recessed-WELL + raised-KEY pair: tracks (switch / segmented / button-group / inputs / checkbox-radio) mill a groove and the active element rises inside it; buttons are seated keys that DEPRESS on `:active`. The single accent follows the scheme. Dark mode = warm graphite (black-ABS Braun). `--ram-bg/-face/-key/-groove/-deep`, `--ram-raise/-sm/-lg`, `--ram-recess/-sm`, `--ram-press`, `--ram-accent`. |
