...
from prompts import discovery            as _discovery       # Phase 3 — discovery flow
from prompts import orchestrator         as _orchestrator    # Phase 5 — workflow-orchestrator skill loader
...
def _scaffold_workflow_for_stages(stages: list, *, has_brand_spec: bool,
                                  has_reference: bool, has_prd_upload: bool,
                                  has_ds_ref: bool, ds_ref_id: str = None,
                                  branch: str = "main",
                                  run_research: bool = True) -> dict:
    """Return a `{ pan, zoom, nodes, edges }` workflow dict for the given
    stage set. Blank scope (stages == []) returns the empty graph.

    Edges are wired between adjacent stages so the user can see the planned
    flow even before any node runs. Port names follow the existing
    convention (`<nodeId>.in` / `<nodeId>.out`) so the workflow surface
    renders bezier curves correctly.
    """
    if not stages:
        return {"pan": {"x": 0, "y": 0}, "zoom": 1, "nodes": [], "edges": []}

    nodes = []
    edges = []
    next_col = 0          # next free stage-column index
    stage_col = {}        # which column each stage's nodes live in

    # Helper: claim a column for a stage, return its index. Used so we can
    # skip columns for stages NOT in the chosen scope.
    def claim_column(stage):
        nonlocal next_col
        stage_col[stage] = next_col
        next_col += 1
        return next_col - 1

    # Helper: wire `from_id.out` → `to_id.in` for every (from, to) pair.
    def wire(*pairs):
        for from_id, to_id in pairs:
            edges.append({"from": f"{from_id}.out", "to": f"{to_id}.in"})

    # ── Upstream context (folder nodes pointing at intake artifacts) ───
    # These live in column 0. Always present when there's any non-A stage.
    # The agent reads brand-spec.md / reference.md / prd.md from these.
    ctx_ids = []
    if has_brand_spec or has_reference or has_prd_upload:
        ctx_col = claim_column("ctx")
        if has_brand_spec:
            nodes.append(_ob_node(
                id="cp_ctx_brandspec", kind="folder", col=ctx_col, row=0,
                title="Intake · brand-spec.md",
                path=f"source/{branch}/brand-spec.md",
            ))
            ctx_ids.append("cp_ctx_brandspec")
        if has_reference:
            nodes.append(_ob_node(
                id="cp_ctx_reference", kind="folder", col=ctx_col, row=1 if has_brand_spec else 0,
                title="Intake · reference.md",
                path=f"source/{branch}/reference.md",
            ))
            ctx_ids.append("cp_ctx_reference")
        if has_prd_upload:
            row = len(ctx_ids)
            nodes.append(_ob_node(
                id="cp_ctx_prd_upload", kind="folder", col=ctx_col, row=row,
                title="Existing · prd.md",
                path=f"source/{branch}/prd.md",
            ))
            ctx_ids.append("cp_ctx_prd_upload")
        if has_ds_ref and ds_ref_id:
            row = len(ctx_ids)
            nodes.append(_ob_node(
                id="cp_ctx_ds_ref", kind="folder", col=ctx_col, row=row,
                title=f"Existing · DS {ds_ref_id}",
                path=f"design-systems/{ds_ref_id}",
            ))
            ctx_ids.append("cp_ctx_ds_ref")

    # ── v2.7b — research node grounds the intake before refinement ───
    # Skip-validate-or-confirm pattern: research finds competitors, audience
    # signals, visual references; refiner THEN expands. Sits between raw
    # intake and bp_brief_refine so the refiner has real signal to chew on
    # instead of LLM intuition alone. Agent kind because it needs WebSearch
    # + WebFetch + multi-step exploration; runs as a focused subprocess.
    # Auto-scaffolded whenever intake ctx exists AND any downstream stage
    # would consume it — same gate as the refiner.
    research_id = None
    if ctx_ids and stages and run_research:
        col_research = claim_column("research")
        research_id = "bp_research"
        nodes.append(_ob_node(
            id=research_id, kind="agent", col=col_research, row=0,
            title="Research + ground the intake",
            conversation=[],
        ))
        for ctx in ctx_ids:
            wire((ctx, research_id))

    # ── v2.10 — brief refiner trio using the existing iterator-refiner ──
    # v2.7 invented a `prompt-refiner` kind that collided with the existing
    # library item "Prompt refiner" (kind: iterator-refiner — a 2-agent
    # interviewer/interviewee loop). v2.10 deletes the invented kind and
    # uses iterator-refiner properly. Because iterator-refiner is driven
    # client-side by setupRefiner (no daemon /run path), the orchestration
    # uses a three-node trio so downstream wiring stays deterministic:
    #
    #   bp_brief_seed   (prompt, empty)   ← orchestrator populates with
    #          ↓                            intake aggregate (brand-spec +
    #                                       reference.md + research.md if on)
    #                                       BEFORE prompting the user to
    #                                       click ✦ Setup loop.
    #   bp_brief_refine (iterator-refiner) ← user clicks "✦ Setup loop";
    #          ↓                            the refiner spawns 5 child nodes
    #                                       and runs the autonomous 2-LLM
    #                                       interview-until-[STOP] loop.
    #   bp_brief_output (prompt, empty)   ← orchestrator copies the refined
    #          ↓                            text here from refiner.outputPromptId
    #          → B + C + E fan out         via POST /__workflow/node/
    #                                       bp_brief_output/status.
    #
    # Downstream stages always wire to bp_brief_output (deterministic id),
    # never to the refiner directly — the refiner's actual output node is
    # spawned at runtime with a uuid-suffixed id we can't pre-wire.
    brief_seed_id   = None
    brief_refine_id = None
    brief_output_id = None
    BRIEF_GOAL = (
        "Reach 9/10 specificity, 9/10 audience-truth precision, 8/10 visual-"
        "intent clarity. Stop when the last three interviewee answers each "
        "satisfy ≥ threshold per criterion. The final refined prompt MUST be "
        "a markdown brief with these sections: One-line product · Audience "
        "truths · Tonal anchors · Surface candidates · Imagery hooks · "
        "Visual identity cues. No section longer than 120 words."
    )
    BRIEF_FOCUS = (
        "Expand the thin intake (App / Audience / Emotion in 3 sentences) "
        "into a structured working brief downstream PRD-refine, DS-brainstorm, "
        "and chunk-PRD nodes can all consume. Walk down: audience situation "
        "→ frustrations → moments-of-need → tonal anchors → surface candidates "
        "→ imagery hooks → visual references. Resolve dependencies (audience "
        "implies surfaces; surfaces imply imagery; imagery implies visual "
        "identity) one at a time."
    )
    BRIEF_PUSHPAST = [
        {"from": "generic personas",       "to": "concrete situations + moments-of-need"},
        {"from": "modern/clean/minimal",   "to": "specific tonal phrases (e.g. 'permission to skip a day')"},
        {"from": "feature lists",          "to": "page-types and their states"},
        {"from": "color/font adjectives",  "to": "named references (apps, sites, artworks)"},
        {"from": "image placeholders",     "to": "mood + subject hooks for imagery generators"},
    ]
    if ctx_ids and stages:
        # Seed (prompt) — orchestrator writes the intake aggregate here at
        # session start, post-research if research is enabled.
        col_seed = claim_column("brief-seed")
        brief_seed_id = "bp_brief_seed"
        nodes.append(_ob_node(
            id=brief_seed_id, kind="prompt", col=col_seed, row=0,
            title="Brief — seed (intake aggregate)",
            text="",
            auto=True,
        ))
        # Refiner (iterator-refiner) — the existing 2-agent library node,
        # pre-configured with onboarding-appropriate goal/focus/pushPast so
        # the user only has to click "✦ Setup loop" to kick it off.
        col_refine = claim_column("brief-refine")
        brief_refine_id = "bp_brief_refine"
        nodes.append(_ob_node(
            id=brief_refine_id, kind="iterator-refiner", col=col_refine, row=0,
            title="Refine brief (2-agent loop)",
            goal=BRIEF_GOAL,
            focus=BRIEF_FOCUS,
            pushPast=BRIEF_PUSHPAST,
            maxTurns=8,
        ))
        # Output sink (prompt) — orchestrator copies the spawned output
        # node's text here after the refinement loop completes. Downstream
        # stages read from this deterministic id, never from the refiner.
        col_output = claim_column("brief-output")
        brief_output_id = "bp_brief_output"
        nodes.append(_ob_node(
            id=brief_output_id, kind="prompt", col=col_output, row=0,
            title="Brief — refined output",
            text="",
            auto=True,
        ))
        # Wiring: seed → refiner → output (the refiner's own port doesn't
        # carry text downstream — the orchestrator does the actual text copy
        # from refiner.outputPromptId → output.text via /status).
        wire((brief_seed_id,   brief_refine_id))
        wire((brief_refine_id, brief_output_id))

    # Helper: when downstream wants intake context, prefer the refined-brief
    # output (deterministic id) over raw ctx folders. Keeps wiring DRY
    # across B / C / E branches.
    def wire_intake_into(target_id):
        if brief_output_id:
            wire((brief_output_id, target_id))
        else:
            for ctx in ctx_ids:
                wire((ctx, target_id))

    # ── B: PRD refinement + downstream text data node ──────────────────
    # v2.4 — every compute→compute hop gets an intermediary data node so the
    # canvas tells the truth about what data flows where. Here:
    #     ctx folders → bp_prd_refine (skill/llm) → bp_prd_text (prompt) → downstream
    # The intermediary is a `prompt` kind because it just carries text; the
    # daemon's upstream walk already reads any kind's .text field on dispatch.
    # Users can inspect AND edit the PRD on the canvas before E (chunks) runs.
    prd_node_id = None
    prd_text_id = None       # data node downstream consumers wire to (preferred)
    if "B" in stages:
        col = claim_column("B")
        prd_node_id = "bp_prd_refine"
        # v2.50 — migrated from skill·llm to agent kind. A full structured PRD
        # (with the mandatory data-model section per the Coherence Pass) is a
        # complex artifact; inline LLM is the wrong dispatch shape (Principle 4).
        nodes.append(_ob_node(
            id=prd_node_id, kind="agent", col=col, row=0,
            title="Refine PRD",
            conversation=[],
            text=("Read the refined brief and write a structured PRD "
                  "(markdown with sections: Problem, Audience, Goals, Pages, "
                  "Key flows, Tone). "
                  # v2.50 — Coherence Pass requirement. The PRD MUST include a "
                  # 'System mechanics + data model' section enumerating every "
                  # named entity, its fields, state transitions, and concrete "
                  # values for the demo scenario. This is the canonical source "
                  # cp_fixture turns into model.json + data.js. Without it, "
                  # pages drift (the `38` vs `312` super bug). See "
                  # editor/kinds/COHERENCE_SCHEMAS.md."
                  "REQUIRED additional section: ## System mechanics + data model "
                  "— enumerate every named entity in the demo (cases, "
                  "incidents, operators, etc.) with their fields and CONCRETE "
                  "demo values, plus the state-transition rules. Every numeric "
                  "or proper-noun fact that appears on more than one page "
                  "MUST be declared here once. Pages quote from this model, "
                  "they don't invent figures. Without this section the "
                  "downstream Coherence Pass cannot run. "
                  "Save to source/<branch>/prd.md."),
        ))
        wire_intake_into(prd_node_id)
        # Intermediary text data node — auto-populated from bp_prd_refine's
        # output on each dispatch. Downstream chunks/refine/update read here.
        col_prd_text = claim_column("B-text")
        prd_text_id = "bp_prd_text"
        nodes.append(_ob_node(
            id=prd_text_id, kind="prompt", col=col_prd_text, row=0,
            title="PRD (text)",
            text="",  # populated by bp_prd_refine's output on the next walk
            auto=True,
        ))
        wire((prd_node_id, prd_text_id))
    elif has_prd_upload:
        # B skipped because user uploaded a PRD — downstream consumers wire
        # against the context folder instead. No intermediary needed; the
        # folder node already exposes the file contents on read.
        prd_node_id = "cp_ctx_prd_upload"
        prd_text_id = "cp_ctx_prd_upload"

    # ── C: DS brainstorm × 3 + per-variant HTML asset + checkpoint ────
    # v2.4 — each ds-brainstorm node has a downstream asset(html) node holding
    # the sample HTML path. The checkpoint reads from the three asset nodes,
    # not from the brainstorm nodes directly. This makes "the HTML that
    # represents variant A" a first-class object on the canvas the user can
    # click on, zoom into, and reopen later via the asset library.
    ds_pick_id = None
    if "C" in stages:
        col = claim_column("C")
        col_html = claim_column("C-html")
        ds_html_ids = []
        for i, suffix in enumerate(("a", "b", "c")):
            bid = f"bs_ds_{suffix}"
            nodes.append(_ob_node(
                id=bid, kind="ds-brainstorm", col=col, row=i,
                title=f"DS variant {suffix.upper()}",
                variant=suffix,
            ))
            wire_intake_into(bid)
            # Downstream asset(html) — the sample HTML for this variant. The
            # path is left empty at scaffold time; the orchestrator fills it
            # in (POST /status with text=<path>) when it writes the sample.
            aid = f"bs_ds_{suffix}_html"
            nodes.append(_ob_node(
                id=aid, kind="asset", col=col_html, row=i,
                title=f"Variant {suffix.upper()} sample",
                assetKind="html",
                path=f"source/{branch}/_ds_brainstorm/{suffix}.html",
            ))
            wire((bid, aid))
            ds_html_ids.append(aid)
        # Checkpoint sits in its own column right after the asset column.
        cp_col = claim_column("C-pick")
        ds_pick_id = "cp_ds_pick"
        nodes.append(_ob_node(
            id=ds_pick_id, kind="agent", col=cp_col, row=0,
            title="Pick DS variant",
            conversation=[],
        ))
        for aid in ds_html_ids:
            wire((aid, ds_pick_id))

    # ── D: DS generation (Workflow 0) — uses the library DS node ──────
    # v2.13a — was a generic `agent` kind with my own per-node preamble;
    # corrected to use the existing `design-system` library node (the "DS
    # library generator" card). That node carries a full spec
    # (genre / tokenPreference / personaModes / primitivePreset / parentRef)
    # and its React component handles the Workflow-0 dispatch via the ▶ Build
    # button. The orchestrator pre-populates genre from the picked variant,
    # then narrates "click ▶ Build" and polls lastRunId — same pattern as
    # the iterator-refiner for bp_brief_refine.
    ds_out_id = None
    if "D" in stages:
        col = claim_column("D")
        ds_out_id = "bp_ds_gen"
        nodes.append(_ob_node(
            id=ds_out_id, kind="design-system", col=col, row=0,
            title="Generate Design System",
            dsId=branch,
            spec={
                "genre":           "",   # orchestrator populates from picked variant's variant-spec JSON
                "tokenPreference": "",
                "personaModes":    [],
                "primitivePreset": [
                    "buttons", "pills", "chips", "tags", "tooltips", "kbd",
                    "inputs", "textarea", "select", "checkbox", "radio", "toggle",
                    "cards", "lists", "tables", "modals", "drawers", "toasts",
                    "badges", "tabs", "menus", "tooltips",
                ],
                "parentRef":       None,
                "extraBrief":      "",
            },
        ...
        if ds_pick_id:
            wire((ds_pick_id, ds_out_id))
        else:
            wire_intake_into(ds_out_id)
    elif has_ds_ref:
        ds_out_id = "cp_ctx_ds_ref"

    # ── E: chunk-PRD → quick HTML × 3 (with v2.4 data nodes) ──────────
    # Compute → data → compute everywhere:
    #   PRD-text → bp_chunks (skill) → bp_chunks_text (prompt, intermediary)
    #     → bs_html_<N> (skill) → bs_html_<N>_asset (asset/html) → F-stage
    chunk_id = None
    chunk_text_id = None
    html_ids = []
    html_asset_ids = []
    if "E" in stages:
        ...
        chunk_id = "bp_chunks"
        nodes.append(_ob_node(
            id=chunk_id, kind="skill", col=col, row=0,
            title="Chunk PRD → 3 pages",
            skill="llm", provider="anthropic", model="claude-opus-4-7",
            text=("Pick 3 representative pages from the PRD that best "
                  "demonstrate the app's range (e.g. login + primary task + "
                  "success state). Write a one-paragraph spec per page. Per "
                  "PRD_VISUAL_RULES, INCLUDE a `shell` field per page "
                  "naming one shell from BRAINSTORM_SHELL_RULES + a one-line "
                  "rationale (e.g. 'centered-narrow because read-decide-act, "
                  "not monitoring'). Also include a `imagery` field per page "
                  "noting which key imagery from the PRD lands on it."),
        ))
        # Source: PRD text node (v2.4 — never the refine node directly).
        # If B was skipped, fall back to the refined brief from v2.7 (or raw
        # ctx folders if even the refiner is absent).
        if prd_text_id:
            wire((prd_text_id, chunk_id))
        else:
            wire_intake_into(chunk_id)
        # Intermediary text data node — chunks output as readable+editable.
        col_chunks_text = claim_column("E-text")
        chunk_text_id = "bp_chunks_text"
        nodes.append(_ob_node(
            id=chunk_text_id, kind="prompt", col=col_chunks_text, row=0,
            title="Chunks (text)",
            text="",
            auto=True,
        ))
        wire((chunk_id, chunk_text_id))
        # HTML-gen × 3, stacked below the chunks-text. Each html node has a
        # downstream asset so the generated file is a first-class object on
        # the canvas (zoomable, library-listed, reopenable).
        col_html = claim_column("E-html")
        col_html_asset = claim_column("E-html-asset")
        for i in range(3):
            hid = f"bs_html_{i+1}"
            # v2.50 — was kind="skill" (inline LLM call, invisible). Each page
            # is a complex artifact — full HTML + the DS + chrome partial +
            # model.json + visual constraints. Per WORKFLOW_TRUTHFULNESS_PLAN.md
            # Principle 4 (complexity → agent kind) and per super's observed
            # failure mode (skill·llm couldn't honor the DS / chrome contract /
            # model.json), this now spawns a visible Claude Code subprocess per
            # page with chat panel, transcript, kill button. The page-specific
            # instructions are in prompts/node_agent_preambles.py per id.
            nodes.append(_ob_node(
                id=hid, kind="agent", col=col_html, row=i,
                title=f"Quick HTML #{i+1}",
                conversation=[],
                # `text` carries the chunked page spec the orchestrator populates
                # via /status. The agent reads it from upstream context AND
                # from its own .text field via the per-id preamble.
                text=_bs_html_default_text(i),
            ))
            wire((chunk_text_id, hid))
            if ds_out_id:
                wire((ds_out_id, hid))
            html_ids.append(hid)
            # Asset sink for the generated HTML — populated by orchestrator
            # POST /status with text=<path> after the html dispatch finishes.
            aid = f"bs_html_{i+1}_asset"
            nodes.append(_ob_node(
                id=aid, kind="asset", col=col_html_asset, row=i,
                title=f"Page {i+1} HTML",
                assetKind="html",
                path=f"source/{branch}/_pages/page_{i+1}.html",
            ))
            wire((hid, aid))
            html_asset_ids.append(aid)

    # ── E.5: Coherence Pass — canonical fixture + chrome + audit lints ──
    # v2.50 — implements COHERENCE_PHASE_PLAN.md. Sits as a sibling pipeline
    # alongside the existing E flow: cp_fixture reads the PRD and emits
    # model.json + data.js; cp_chrome reads the DS and emits chrome.html +
    # chrome.contract.json; both feed into the html generators so prose
    # never invents facts and chrome never diverges. Downstream lints +
    # gate close the loop before the prototype is released. Gated on E
    # because there's nothing to lint without pages.
    coherence_lint_data_id   = None
    coherence_lint_chrome_id = None
    coherence_gate_id        = None
    if "E" in stages and html_asset_ids:
        # cp_fixture — between PRD-text and the html generators. ALSO
        # writes data.js so the canonical store is referenceable in JS.
        col_cf = claim_column("E-fixture")
        cp_fixture_id = "cp_fixture"
        nodes.append(_ob_node(
            id=cp_fixture_id, kind="agent", col=col_cf, row=0,
            title="Canonical fixture (model.json + data.js)",
            conversation=[],
        ))
        # Wire from PRD-text (the data node, not the producer)
        if prd_text_id:
            wire((prd_text_id, cp_fixture_id))
        else:
            wire_intake_into(cp_fixture_id)
        # And feed into the html generators so they see model.json upstream.
        for hid in html_ids:
            wire((cp_fixture_id, hid))
        # Also feed bp_chunks if present, so chunk-spec authors can
        # reference the same entities.
        if chunk_id:
            wire((cp_fixture_id, chunk_id))

        # cp_chrome — between DS and html generators. Only meaningful if
        # there's a DS upstream.
        if ds_out_id:
            col_cc = claim_column("E-chrome")
            cp_chrome_id = "cp_chrome"
            nodes.append(_ob_node(
                id=cp_chrome_id, kind="agent", col=col_cc, row=0,
                title="Shared chrome contract",
                conversation=[],
            ))
            wire((ds_out_id, cp_chrome_id))
            for hid in html_ids:
                wire((cp_chrome_id, hid))

        # Downstream audits — lint_data_coherence + lint_chrome_consistency.
        # Both read every html asset's output and append to COHERENCE_REPORT.json.
        col_lint = claim_column("E-lint")
        coherence_lint_data_id = "lint_data_coherence"
        nodes.append(_ob_node(
            id=coherence_lint_data_id, kind="agent", col=col_lint, row=0,
            title="Lint · data coherence",
            conversation=[],
        ))
        for aid in html_asset_ids:
            wire((aid, coherence_lint_data_id))
        wire((cp_fixture_id, coherence_lint_data_id))   # lint reads the model.json contract

        coherence_lint_chrome_id = "lint_chrome_consistency"
        nodes.append(_ob_node(
            id=coherence_lint_chrome_id, kind="agent", col=col_lint, row=1,
            title="Lint · chrome consistency",
            conversation=[],
        ))
        for aid in html_asset_ids:
            wire((aid, coherence_lint_chrome_id))
        if ds_out_id:
            # The chrome lint reads cp_chrome's contract, not the DS directly.
            wire(("cp_chrome", coherence_lint_chrome_id))

        # cp_coherence_gate — final release decision. Reads both lint
        # outputs (and any v_<id> verifies the visual-planner appends
        # dynamically via /commit addNodes). Writes DECISION_cp_coherence.json
        # and emits <decision-request> on block findings.
        col_gate = claim_column("E-gate")
        coherence_gate_id = "cp_coherence_gate"
        nodes.append(_ob_node(
            id=coherence_gate_id, kind="agent", col=col_gate, row=0,
            title="Coherence gate · release decision",
            conversation=[],
        ))
        wire((coherence_lint_data_id,   coherence_gate_id))
        wire((coherence_lint_chrome_id, coherence_gate_id))

    # ── F: remix per page — 3 nodes × n:3 = 9 alternatives total ──────
    # v2.4 — corrected from v1's 9 flat nodes. Each remix takes ONE page and
    # produces N alternatives of that page. The downstream html-set asset
    # node holds the 3 paths so the checkpoint can render them as a row.
    #     bs_html_<N>_asset → br_remix_p<N> (n:3) → br_remix_p<N>_set (asset)
    #                                                  → cp_remix_pick
    # v2.14a — pre-fill `variants` with 3 template direction-guidance strings.
    # v2.19b — the daemon /run gate refuses dispatch when variants still equal
    # the scaffolder defaults; orchestrator MUST override via POST /status with
    # picked-DS-aware specifics before calling /run. Defaults are kept (rather
    # than empty) so a manual user click on Run shows what direction-shape is
    # expected — but they're not the live runtime.
    remix_set_ids = []
    if "F" in stages:
        col = claim_column("F")
        col_set = claim_column("F-set")
        for h_idx in range(3):
            rid = f"br_remix_p{h_idx + 1}"
            nodes.append(_ob_node(
                id=rid, kind="iterator-remix", col=col, row=h_idx,
                title=f"Remix page {h_idx + 1} (3 alts)",
                n=3,  # produces 3 alternatives per page
                variants=list(REMIX_VARIANT_DEFAULTS),
            ))
            # Each remix takes its corresponding page's HTML asset as input.
            if h_idx < len(html_asset_ids):
                wire((html_asset_ids[h_idx], rid))
            if ds_out_id:
                wire((ds_out_id, rid))
            # html-set sink — holds the 3 alt paths for this page so the
            # checkpoint can render them as a row in the 3×3 grid.
            sid = f"br_remix_p{h_idx + 1}_set"
            nodes.append(_ob_node(
                id=sid, kind="asset", col=col_set, row=h_idx,
                title=f"Page {h_idx + 1} alts (3)",
                assetKind="html-set",
                # Paths populated by orchestrator after writing the alts.
                paths=[
                    f"source/{branch}/_remix/p{h_idx+1}_a.html",
                    f"source/{branch}/_remix/p{h_idx+1}_b.html",
                    f"source/{branch}/_remix/p{h_idx+1}_c.html",
                ],
            ))
            wire((rid, sid))
            remix_set_ids.append(sid)

    # ── G: pick best remix + refine PRD ───────────────────────────────
    # v2.4 — cp_remix_pick reads from the per-page html-set assets (not
    # individual remix nodes). bp_prd_final reads from the PRD text node
    # (bp_prd_text), not directly from bp_prd_refine.
    final_prd_id = None
    final_prd_text_id = None
    if "G" in stages:
        col_pick = claim_column("G-pick")
        cp_remix = "cp_remix_pick"
        nodes.append(_ob_node(
            id=cp_remix, kind="agent", col=col_pick, row=0,
            title="Pick best feel",
            conversation=[],
        ))
        for sid in remix_set_ids:
            wire((sid, cp_remix))
        col_refine = claim_column("G-refine")
        final_prd_id = "bp_prd_final"
        # v2.50 — migrated to agent kind (same rationale as bp_prd_refine).
        nodes.append(_ob_node(
            id=final_prd_id, kind="agent", col=col_refine, row=0,
            title="Refine PRD with pick",
            conversation=[],
            text=("Read the picked variant; update prd.md with the direction "
                  "it implies. Apply REFINER_VISUAL_AXIS — score on at least "
                  "one visual axis, offer one visual-push option. Preserve "
                  "the four PRD_VISUAL_RULES sections. "
                  # v2.50 — Coherence Pass requirement: preserve the "
                  # 'System mechanics + data model' section verbatim or "
                  # refine it to match the picked direction's surfaces. "
                  # Never delete it.
                  "PRESERVE (or refine) the '## System mechanics + data "
                  "model' section — never drop it. The downstream cp_fixture "
                  "node consumes it; without it, pages will drift across "
                  "surfaces. If the picked direction implies new entities or "
                  "fields, ADD them to the model section before they appear "
                  "in page prose anywhere."),
        ))
        wire((cp_remix, final_prd_id))
        if prd_text_id:
            wire((prd_text_id, final_prd_id))
        # Intermediary text data node for the refined PRD.
        col_final_text = claim_column("G-text")
        final_prd_text_id = "bp_prd_final_text"
        nodes.append(_ob_node(
            id=final_prd_text_id, kind="prompt", col=col_final_text, row=0,
            title="Refined PRD (text)",
            text="",
            auto=True,
        ))
        wire((final_prd_id, final_prd_text_id))

    # Pick the freshest PRD text node available as the downstream source.
    downstream_prd_text = final_prd_text_id or prd_text_id

    # ── H: DS update from PRD (Workflow 6+6b) ─────────────────────────
    if "H" in stages:
        col = claim_column("H")
        nodes.append(_ob_node(
            id="bp_ds_update", kind="agent", col=col, row=0,
            title="Update DS from PRD",
            conversation=[],
        ))
        if downstream_prd_text:
            wire((downstream_prd_text, "bp_ds_update"))
        if ds_out_id:
            wire((ds_out_id, "bp_ds_update"))

    # ── H2: realign PRD with updated DS (v2.15) ───────────────────────
    # H rewrites the DS based on the refined PRD — that can add new
    # components, rename tokens, change the page-to-shell map. Without
    # this step, I (prototype build) would see a PRD that still says
    # "card" while the DS now ships a stricter `DataCard`. H2 rewrites
    # the per-screen "components used" + token references in the PRD so
    # they speak the DS's canonical names. Skill=llm — one shot — reads
    # bp_prd_final_text + the bp_ds_update result, writes prd.md in
    # place, output flows into bp_prd_align_text.
    align_prd_id = None
    align_prd_text_id = None
    if "H" in stages:
        col = claim_column("H2")
        align_prd_id = "bp_prd_align"
        # v2.50 — migrated to agent kind (same rationale as bp_prd_refine).
        nodes.append(_ob_node(
            id=align_prd_id, kind="agent", col=col, row=0,
            title="Realign PRD with updated DS",
            conversation=[],
            text=("The DS was just updated (see upstream `Update DS from PRD`). "
                  "Rewrite the refined PRD so per-screen component references, "
                  "token names, and the page-to-shell map all use the DS's "
                  "canonical names. Do NOT change the product intent or the "
                  "page list — only realign vocabulary. Preserve the four "
                  "PRD_VISUAL_RULES sections. "
                  # v2.50 — Coherence Pass requirement: data-model section
                  # is non-negotiable through every PRD refinement step.
                  "PRESERVE the '## System mechanics + data model' section "
                  "verbatim — its entity ids and concrete values are now "
                  "referenced by data.js and every page's prose. Renaming "
                  "or removing an entity here breaks the canonical fixture. "
                  "Output the updated PRD body."),
        ))
        if downstream_prd_text:
            wire((downstream_prd_text, align_prd_id))
        wire(("bp_ds_update", align_prd_id))
        col_align_text = claim_column("H2-text")
        align_prd_text_id = "bp_prd_align_text"
        nodes.append(_ob_node(
            id=align_prd_text_id, kind="prompt", col=col_align_text, row=0,
            title="Realigned PRD (text)",
            text="",
            auto=True,
        ))
        wire((align_prd_id, align_prd_text_id))

    # Downstream consumers (I, J) prefer the realigned PRD when H was
    # in scope — it has the matching DS vocabulary. Fall back to the
    # refined-PRD text otherwise.
    proto_prd_text = align_prd_text_id or downstream_prd_text

    # ── I: prototype build (Workflow 1) ───────────────────────────────
    if "I" in stages:
        col = claim_column("I")
        nodes.append(_ob_node(
            id="bp_proto_build", kind="agent", col=col, row=0,
            title="Build prototype",
            conversation=[],
        ))
        if proto_prd_text:
            wire((proto_prd_text, "bp_proto_build"))
        if "H" in stages:
            wire(("bp_ds_update", "bp_proto_build"))
        elif ds_out_id:
            wire((ds_out_id, "bp_proto_build"))

    # ── J: design brief (v2.8) ────────────────────────────────────────
    # Optional trailing stage that produces DESIGN_BRIEF.html at project
    # root — convincing case for the design (9 sections incl. storyboard,
    # per-screen rationale, rejected-and-why). Only meaningful when a
    # prototype exists (depends on I); the wizard's Custom toggle is gated
    # on buildPrototype. Reads from EVERYTHING — final PRD, updated DS,
    # source/, picked decisions, brainstorm + remix HTMLs.
    if "J" in stages and "I" in stages:
        col = claim_column("J")
        nodes.append(_ob_node(
            id="bp_design_brief", kind="agent", col=col, row=0,
            title="Design brief + storyboard",
            conversation=[],
        ))
        wire(("bp_proto_build", "bp_design_brief"))
        # Also wire PRD + DS so the brief subagent has them as upstream
        # context without re-reading source files.
        if proto_prd_text:
            wire((proto_prd_text, "bp_design_brief"))
        if "H" in stages:
            wire(("bp_ds_update", "bp_design_brief"))
        elif ds_out_id:
            wire((ds_out_id, "bp_design_brief"))

    return {"pan": {"x": 0, "y": 0}, "zoom": 0.9, "nodes": nodes, "edges": edges}
...
class H(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if url_path == "/__onboarding/status":
            return self._onboarding_status(urllib.parse.parse_qs(parsed.query))
    ...
    # ── GET /__onboarding/status?project=<id> ───────────────────────────
    # Phase 6 — the WorkflowCanvas calls this on mount to decide whether to
    # auto-spawn the orchestrator chat. Returns the parsed marker contents so
    # the UI can also display the scope/stages summary (and Phase 7 can show
    # the Resume? banner driven off completedStages).
    def _onboarding_status(self, qs):
        try:
            project_root = resolve_project_root(qs, require_explicit=True)
        except ValueError as e:
            return self._reply(400, {"error": str(e)})
        marker_path = os.path.join(project_root, ".onboarding-pending")
        if not os.path.isfile(marker_path):
            return self._reply(200, {"pending": False})
        try:
            with open(marker_path, "r", encoding="utf-8") as f:
                marker = json.load(f)
        except (OSError, ValueError) as e:
            return self._reply(200, {"pending": True, "error": f"marker unreadable: {e}"})
        if not isinstance(marker, dict):
            return self._reply(200, {"pending": True, "error": "marker is not a JSON object"})
        return self._reply(200, {"pending": True, "marker": marker})
    ...
    # POST /__projects/new  body: { id, label?, template? }
    # Scaffolds <WORKSPACE_DIR>/projects/<id>/{source/main/, editor/branches/
    # main.js, editor/data.js}. Only valid in workspace mode. The template arg
    # is reserved for future use (Phase 6b); for now only "blank" is supported.
    def _project_create(self, qs):
        ...
        # Phase-6.2 — new projects go under projects/<id>/. Create the parent
        # dir lazily so the workspace can be set up incrementally.
        ...
        # v3.1 — the bootstrap shim is gone (editor/data.js carries
        # window.EDITOR_DATA directly, no editor/branches/<slug>.js sidecar)
        # but the on-disk source tree is STILL `source/<slug>/` with the
        # default slug `main`. Every other endpoint (uploads, /__doc, the
        # workflow ctx-node paths) assumes that layout; an earlier attempt
        # to flatten it to a bare `source/` here was incomplete and broke
        # onboarding-artifact writes (brand-spec.md / reference.md silently
        # failed because `source/main/` didn't exist).
        try:
        ...
        # ── Onboarding artifacts (Phase 1) ────────────────────────────────
        # Optional fields land here only if the new wizard supplied them.
        # Blank-scope projects (or pre-Phase-1 callers that only sent id+label)
        # take the early-return path WITHOUT writing brand-spec.md or
        # .onboarding-pending — they get the legacy behavior.
        ...
        reference = body.get("reference") if isinstance(body.get("reference"), dict) else None
        existing  = body.get("existing")  if isinstance(body.get("existing"),  dict) else None
        stages    = (scope.get("stages") if scope and isinstance(scope.get("stages"), list) else []) or []

        # Don't fail the whole create if an artifact write hits an error; the
        # project still exists, and the user can re-trigger orchestration via
        # the Phase 7 menu later.
        onboarding_warnings = []
        src_main = os.path.join(dest, "source", "main")

        if intent:
            try:
                brand_md = (
                    f"# Brand spec — {label}\n\n"
                    f"_Captured by the new-project wizard on "
                    f"{_dt.datetime.now().isoformat(timespec='seconds')}._\n\n"
                    f"## App\n\n{(intent.get('app') or '').strip() or '_(empty)_'}\n\n"
                    f"## Audience\n\n{(intent.get('audience') or '').strip() or '_(empty)_'}\n\n"
                    f"## Emotion\n\n{(intent.get('emotion') or '').strip() or '_(empty)_'}\n"
                )
                with open(os.path.join(src_main, "brand-spec.md"), "w", encoding="utf-8") as f:
                    f.write(brand_md)
            except OSError as e:
                onboarding_warnings.append(f"brand-spec.md: {e}")

        if reference:
            try:
                mode = (reference.get("mode") or "brainstorm").strip()
                ref_lines = [f"# Reference\n\nMode: **{mode}**\n"]
                url = (reference.get("url") or "").strip()
                txt = (reference.get("text") or "").strip()
                if url: ref_lines.append(f"\nURL: {url}\n")
                if txt: ref_lines.append(f"\n## Free-text vibe\n\n{txt}\n")
                # Save screenshots to _attachments/ AND list their paths here.
                shots = reference.get("screenshots") or []
                if isinstance(shots, list) and shots:
                    attach_dir = os.path.join(src_main, "_attachments")
                    os.makedirs(attach_dir, exist_ok=True)
                    ref_lines.append("\n## Screenshots\n")
                    for i, s in enumerate(shots):
                        if not isinstance(s, dict): continue
                        data_uri = s.get("dataUri") or ""
                        ...
                        base = _slugify(os.path.splitext(s.get("name", "shot"))[0]) or f"shot-{i+1}"
                        fname = f"ref-{int(time.time()*1000)}-{base}.{ext}"
                        try:
                            with open(os.path.join(attach_dir, fname), "wb") as f:
                                f.write(raw)
                            ref_lines.append(f"- `_attachments/{fname}` — {s.get('name', '')}\n")
                        except OSError as e:
                            onboarding_warnings.append(f"attachment {fname}: {e}")
                with open(os.path.join(src_main, "reference.md"), "w", encoding="utf-8") as f:
                    f.write("".join(ref_lines))
            except Exception as e:
                onboarding_warnings.append(f"reference.md: {type(e).__name__}: {e}")

        # Existing PRD upload — write to source/main/prd.md so downstream
        # stages (E/F/G/H/I) can read it as the input.
        if existing and isinstance(existing.get("prd"), dict):
            try:
                prd = existing["prd"]
                prd_text = (prd.get("text") or "").strip()
                if prd_text:
                    with open(os.path.join(src_main, "prd.md"), "w", encoding="utf-8") as f:
                        f.write(prd_text + ("\n" if not prd_text.endswith("\n") else ""))
            except OSError as e:
                onboarding_warnings.append(f"prd.md: {e}")

        # Existing DS pointer — write a marker file (workflow Phase 2 will
        # read this when scaffolding nodes so the agent knows which DS to wire).
        if existing and isinstance(existing.get("ds"), dict):
            try:
                ds = existing["ds"]
                if ds.get("kind") == "workspace" and (ds.get("id") or "").strip():
                    with open(os.path.join(dest, ".onboarding-ds-ref"), "w", encoding="utf-8") as f:
                        f.write(json.dumps({"kind": "workspace", "id": ds["id"].strip()}, indent=2))
                # Upload-trio kind is Phase 1-deferred; flagged in the wizard UI.
            except OSError as e:
                onboarding_warnings.append(f"ds-ref marker: {e}")

        # The orchestrator marker — Phase 5 reads this on every workflow-mode
        # chat to know what to do. Blank scope (or no scope at all) leaves
        # this absent so Phase 6's auto-launch is also a no-op.
        if stages:
            try:
                marker = {
                    "createdAt": _dt.datetime.now().isoformat(timespec="seconds"),
                    "scope":     scope.get("id") or "custom",
                    "stages":    stages,
                    "options":   scope.get("options") or {},
                    "completedStages": [],
                    "pendingDecisions": [],
                    "inputs": {
                        "hasBrandSpec": bool(intent),
                        "hasReference": bool(reference),
                        "hasPrdUpload": bool(existing and isinstance(existing.get("prd"), dict)),
                        "hasDsRef":     bool(existing and isinstance(existing.get("ds"),  dict)),
                    },
                }
                with open(os.path.join(dest, ".onboarding-pending"), "w", encoding="utf-8") as f:
                    json.dump(marker, f, indent=2)
            except OSError as e:
                onboarding_warnings.append(f".onboarding-pending: {e}")

            # Phase 2 — scaffold workflow.json with only the nodes the
            # chosen stages need. Layout is deterministic so users see the
            # same shape every time. Blank scope is excluded by the outer
            # `if stages:` — Blank gets no workflow.json scaffold.
            try:
                ds_ref_id = None
                if existing and isinstance(existing.get("ds"), dict) and existing["ds"].get("kind") == "workspace":
                    ds_ref_id = existing["ds"].get("id")
                # v2.7c — runResearch is a scope OPTION (not a stage).
                # Default true if the wizard didn't set it; explicit false
                # skips bp_research scaffolding entirely.
                scope_options = (scope.get("options") if isinstance(scope.get("options"), dict) else {}) or {}
                run_research = scope_options.get("runResearch", True) is not False
                wf = _scaffold_workflow_for_stages(
                    stages,
                    has_brand_spec = bool(intent),
                    has_reference  = bool(reference),
                    has_prd_upload = bool(existing and isinstance(existing.get("prd"), dict)),
                    has_ds_ref     = bool(ds_ref_id),
                    ds_ref_id      = ds_ref_id,
                    branch         = "main",
                    run_research   = run_research,
                )
                wf_dir = os.path.join(dest, "workflow")
                os.makedirs(wf_dir, exist_ok=True)
                with open(os.path.join(wf_dir, "workflow.json"), "w", encoding="utf-8") as f:
                    json.dump(wf, f, indent=2)
            except Exception as e:
                onboarding_warnings.append(f"workflow scaffold: {type(e).__name__}: {e}")

        # Persist the label in workspace.json so /__projects reports it back
        # (auto-discovery would otherwise fall back to id == label). Create the
        # file if missing.
        ws_json = os.path.join(WORKSPACE_DIR, "workspace.json")
        try:
            cfg = {}
        ...
        reply = {"ok": True, "id": proj_id, "label": label, "path": dest}
        if stages:           reply["stages"] = stages
        if onboarding_warnings: reply["onboardingWarnings"] = onboarding_warnings
    ...
    def _run_create(self, qs):
        ...
        wants_discovery = _discovery.wants_discovery(kind, project_root, branch,
                                                     user_prompt=user_prompt)
        # Phase 5 — when `.onboarding-pending` exists at project root, the
        # user picked a non-Blank scope at creation and the workflow chat is
        # supposed to auto-drive the pre-scaffolded graph. Mount the
        # workflow-orchestrator skill ahead of discovery so the agent's first
        # turn reads the marker and starts the sequence instead of ambushing
        # the user with a brief form they already filled out in the wizard.
        wants_orchestration = _orchestrator.wants_orchestration(project_root)
        include_views = _discovery.wants_editor_views(kind)
        if agent_id == "claude":
            sys_prompt = QUESTION_FORM_SYSTEM_PROMPT
            if WORKSPACE_DIR and project_root != INSTALL_ROOT:
            ...
            if wants_orchestration:
                orch = _orchestrator.orchestrator_prompt(
                    project_root, INSTALL_ROOT, branch=branch,
                )
                if orch:
                    sys_prompt = sys_prompt + "\n\n" + orch
            # Phase 3 — empty-source freeform run or new fork: prepend the
            # discovery RULE 1/2 block so the agent emits the brief form on
            # turn 1 instead of guessing a direction. The editor-view picker
            # is appended only for view-populating workflows (regenerate);
            # plain freeform / fork builds get the core questions only.
            # Suppressed when orchestration is active (see above).
            elif wants_discovery:
                sys_prompt = sys_prompt + _discovery.discovery_rules_prompt(
                    branch, include_views=include_views,
                )
