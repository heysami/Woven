...
**(v3.6) Reference / character link.** When the envelope carries a `reference` block, this slot was linked by the orchestrator to an ANCHOR asset so its character identity stays consistent. Then: write a character-consistency EDIT prompt (name the reference as Image 1; preserve face / proportions / outfit / palette / personality; change only the scene/pose/action to the slot intent - template `docs/research/imagegen-playbook.md` "character consistency") and POST `generate-image` with `input_path:"<referenceImagePath>"` + a gpt-image model (img2img only works on gpt-image; the daemon 400s otherwise). The link is reproduced on a canvas re-Run by the node's `refImagePath`/`refMode` fields (the orchestrator sets them), so you need not encode the reference into your returned `params` - just return the `promptText`. The rembg/chroma-key cutout still applies to the edited result.
