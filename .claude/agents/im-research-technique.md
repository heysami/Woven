...
Prose, not a word-list. Write the briefs for this piece's builders in the vocabulary interaction-design work actually uses to describe WHAT THIS PIECE DOES - the input→mapping→output chain as a designer of motion and interaction would name it: how the input maps to the response, the latency budget the response has to hit, whether the response pursues its target with critical damping or snaps to it, the gesture that drives it, the weight and momentum the response carries, the settle after release. DERIVE the actual terms from the committed `mappingStyle` + the committed input/output modalities + the latency targets in this project - do NOT reach for a fixed house vocabulary. The point is that "the cursor drags a damped trail that overshoots then settles" carries the bar in a way "things move when you move the mouse" never can *(illustrative, non-binding: the exact words must come from what THIS chain does)*. This is a prose lane, judged holistically at the lens gate - never a checklist, never diffed against a target string.

## Principle stance (`principleStance`)
Prose of what "real" means for this specific piece, phrased for holistic lens judgment rather than as a checklist. State the bar in its own terms: the response lands within the committed latency budget so the piece feels connected to the hand, not laggy; the motion is damped rather than snapped where the committed mappingStyle calls for weight; the mapping is non-trivial against the brief (it does something more than a linear pass-through when the successFeel demands surprise); and the whole feel matches the committed `successFeel`. Keep it a paragraph of intent the lens reads and judges as a whole *(illustrative, non-binding)* - it is NOT a pass/fail rubric and is never diffed line by line.
...
In the SAME pass as `research.md`, write `test-cases.json` NEXT TO it (`source/{branch}/interactives/{imId}/test-cases.json`). The QA gate (`GET /__qa/run`) auto-detects the file and walks EVERY case end to end instead of the generic battery: the plan defines its own proof. Schema + runner: `docs/features/qa-test-cases.md` and `editor/tools/qa/README.md` (self-test fixture: `editor/tools/qa/fixtures/cases-demo.test-cases.json`).

Required contents:

- `harness`: the piece's devtools global (`window.__im` - the §12.3 harness im-runtime-composer exposes).
- `phases`: every state the piece can be in. `intents`: every user action, using the EXACT intent kinds the input layer emits.
...
- `cases` (journeys): the full happy path start to finish, EVERY terminal outcome (not just the happy one), and restart-after-end. Cases MUST cover each declared input granted AND denied (the permission-fallback path is the classic crash site), driven via `intent` steps that inject synthetic feature vectors. Plus any state x input combination you already see being fragile.
