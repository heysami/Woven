...
<decision-request id="cp_polish_research_pick_<polishId>" requires="value">
  ...
  <details>
    ...
    Per-drawer dispatch cost: ~<N> drawer runs (NO per-drawer lens) + ~<M> visual-orchestrator sub-dispatches for shaders + ONE final QA+lens gate on the polished host pages (~3-9 lens runs across ≤3 gate iterations).
...
8. **`qa_gate_<polishId>`** (kind: `agent`) - the final-gate node (registry `qa_gate_` override). Scaffold it right after the container; do NOT dispatch it yourself. **Ordering nuance (unlike the other families): the caller does NOT blindly auto-chain the gate after the drawers either** - the gate judges the polished HOST PAGES, so it fires only AFTER the caller applies `integration-instructions.md`'s minimal HTML edits (§7.2 step 2). Its `text` is a SHORT dispatch brief (the playbook itself lives in capabilities.py - reference, don't restate):

   > You are the final QA+lens gate for container `polish_<polishId>` (family: interactive-polish, slotId: `<polishId>`, prototype: `<branch>`). Read `$TH_PROTOCOL_ROOT/editor/kinds/capabilities.py` "Three contracts of the orchestrator family" contract 3 FROM DISK now and follow it verbatim, with this family's substance: there is NO iframe container - the gate target is the polished HOST PAGES (the caller has already applied `integration-instructions.md`'s `<link>`/`<script>` edits). Run interactive-polish-orchestrator.md §8's per-site checks against every host page listed in `source/<branch>/_polish/<polishId>/polish-plan.json` (each site's effect fires, console + network clean, byte budgets, cross-page consistency, genre fit, NO host-content breakage), plus a promised-vs-shipped diff against `polish-plan.json` + `research.md`, then the lens trio AS WORKFLOW NODES - `addNodes [craft_lens_<polishId>_<iter>, aesthetic_lens_<polishId>_<iter>, concept_lens_<polishId>_<iter>]` + `POST /run` in parallel, NEVER the Task tool - verdicts from `QUALITY_REPORT.json`, code fixes routed through `solution-proposer` + re-dispatch of the responsible drawers, re-run `polish_runtime_<polishId>` to re-compose, re-gate (max 3 outer iterations). Write the `qa` block to `workflow/polish-plan.json`. On pass: `POST /__workflow/node/polish_<polishId>/commit` with `outputs.lensVerdict=pass`, `outputs.siteCount`, `outputs.appliedPages`, `runStatus=done`. At the cap: put the `<decision-request id="cp_polish_gate_<polishId>">` block (Accept / Replace / Drop-site) in your FINAL MESSAGE - a node run cannot render chat cards; the caller relays it verbatim.
...
| `kind` | yes | `"agent"` for drawers + the qa_gate node; `"interactive-polish"` for the container. |
...
```jsonc
{
  ...
  "nextStep": "Caller auto-chains scaffold.drawerNodes[] in order with NO per-drawer lens (drawers commit on file-existence; the runtime drawer writes composite.css + composite.js + integration-instructions.md, LAST). Caller then APPLIES integration-instructions.md's edits to source/<branch>/*.html (this is the one HTML edit polish-orchestrator authorises - adding a single supplemental stylesheet + script ref per host page). ONLY THEN caller dispatches scaffold.gateNode (qa_gate_<polishId>) - the gate judges the polished HOST PAGES, so it must fire AFTER the integration edits, never auto-chained past them. The gate runs the SINGLE final QA+lens gate as its own leaf run (§8 substance), writes the qa block, and commits scaffold.containerNode; caller RELAYS any <decision-request> block from the gate node's output verbatim."
}
```

### 7.1 What the caller does next

Per the build harness pseudocode in §7.2, auto-chain the drawers in order with **NO per-drawer lens** - drawers commit on file-existence; quality is judged ONCE by the final gate. After every drawer completes, READ the runtime drawer's `integration-instructions.md` and APPLY the minimal HTML edits to each host page (a single `<link rel="stylesheet" href="_polish/<polishId>/composite.css">` and `<script src="_polish/<polishId>/composite.js" defer></script>` per page, plus an optional `<div data-polish-shader-mount></div>` if shader is used). DO NOT modify any content/structure of the host page - only insert the polish refs. ONLY THEN dispatch `qa_gate_<polishId>` - it judges the polished host pages, runs the single final QA+lens gate as its own leaf run, and commits the container.

### 7.2 Build harness pseudocode

```
# 1. AUTO-CHAIN the drawers - NO per-drawer lens, NO chat turn between links.
#    Drawers commit on file-existence; runtime/composer is the LAST link.
#    The gate is NOT in this chain - it fires only after step 2's integration edits.
#    Exception: if polish_shader_<polishId> is in scaffold.multiDraftCruxes, chain
#    only up to it - the 3 drafts commit to _polish/<polishId>/_shader_remix/{va,vb,vc}/;
#    scaffold cp_polish_shader_pick_<polishId>, user picks, copy the picked variant
#    to the canonical path - then chain the remaining drawers.
first = scaffold.drawerNodes[0]
rest  = scaffold.drawerNodes[1:]
POST /__workflow/node/<first>/run?chain=<comma-joined rest>
poll until polish_runtime_<polishId> is done/error   # if a drawer errors the chain halts there - surface it

# 2. APPLY the integration edits BEFORE the gate - the gate judges the polished
#    HOST PAGES; unedited pages give it nothing to judge.
...
# 3. Dispatch the gate node (never auto-chained past step 2)
POST /__workflow/node/qa_gate_<polishId>/run
poll until qa_gate_<polishId> is done/error

# 4. The gate runs the SINGLE final QA+lens gate AS ITS OWN LEAF RUN
#    (capabilities.py contract 3; substance = this file's §8 per-site checks on the
#    polished host pages + promised-vs-shipped diff + lens trio AS NODES +
#    solution-proposer + drawer re-dispatch + composer re-run, max 3 iterations).
#    On pass it writes the qa block to workflow/polish-plan.json and commits
#    polish_<polishId> with outputs.lensVerdict=pass. NOT in this thread.
# 5. When the gate lands:
#    done  -> if its output carries a <decision-request> (iteration cap:
#             cp_polish_gate_<polishId>), RELAY the block VERBATIM in your reply -
#             only the chat can render the card - then honour the pick. Otherwise
#             relay the pass summary.
#    error -> surface the gate node's runError; ONLY then may you run the §8 gate
#             loop inline as the legacy fallback - and say so out loud.
```

### 7.3 Multi-draft (§8.7) - only the shader crux

Most polish drawers don't benefit from multi-draft - microanimation, pointer, hover are all "pick the right specific behavior" and the final gate catches misses. The ONE exception:
...
## 8. Phase E - Final QA+lens gate substance (runs INSIDE `qa_gate_<polishId>`)

After every drawer is `done` + the host page edits are applied, the caller dispatches `qa_gate_<polishId>`. Everything below is the GATE NODE's playbook substance (capabilities.py contract 3, with this family's checks) - it runs at the gate node's fresh leaf context, not the caller's thread. Chat-inline execution of this loop is the legacy FALLBACK, permitted only when dispatching the gate node itself errors - and say so out loud. The gate commits the container on pass:
...
11. **Then the lens trio, ONE set, on the polished result.** `addNodes [craft_lens_<polishId>_<iter>, aesthetic_lens_<polishId>_<iter>, concept_lens_<polishId>_<iter>]` + `POST /run` in parallel - AS WORKFLOW NODES, never Task; verdicts from `QUALITY_REPORT.json`. QA pass + ≥2 lens pass → `POST /__workflow/node/polish_<polishId>/commit` with `outputs.lensVerdict=pass`, `runStatus=done`. Fail → route code fixes through `solution-proposer`, re-dispatch the responsible drawer (step 9's levers), re-run `polish_runtime_<polishId>` to re-compose, re-gate - max 3 outer iterations. At the cap, put `<decision-request id="cp_polish_gate_<polishId>">` (Accept / Replace / Drop-site) in the gate node's FINAL MESSAGE for the caller to relay verbatim.
...
- **You do not run lens trios.** Quality is judged ONCE, by the chained `qa_gate_<polishId>` node's single final QA+lens gate on the polished host pages - never per drawer.
- **You do not commit the `polish_<polishId>` container.** The `qa_gate_<polishId>` node commits it when the gate passes.
...
| §7.1 (caller) | drawer per opportunity type (auto-chain, NO per-drawer lens) | CALLER | drawer dispatch, commit-on-file-existence | done | (n/a) |
| §7.1 (caller) | apply integration HTML edits | CALLER | per-page Edit | - | - |
| §7.1 (caller) | `qa_gate_<polishId>` dispatched AFTER the edits | CALLER | node /run | done | - |
| §8 (gate node) | `polish_<polishId>` (container) | GATE NODE | direct, after the final QA+lens gate passes | done | `pass` |
...
> **Architectural note (do not edit this section out).** The harness pseudocode (drawer auto-chain, integration edits, gate-node dispatch, §7.3 multi-draft shader crux) lives in §7.2. The caller (workflow-mode chat) reads it to drive the build. Do NOT add a Phase D *drive-the-build-yourself* section. The polish pass touches host pages - those edits MUST happen in the chat-Claude's authorised session, not in this cold subagent.
