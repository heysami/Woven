...
1.5. **Write a `sketch` (project-local imperative code layer) - the DEFAULT escape hatch for novel interaction.** When the slot is a per-pixel / temporal / stateful interaction the primitives cannot express (slit-scan, ring-buffer trails, custom pointer mappings, particle toys), do NOT jump to a global engine edit. Use the `sketch` kind: write arbitrary JS (`export function draw(ctx, frame, controls, content)`, optional `setup`) to `source/<branch>/sketch-<id>.js`. It runs in a SANDBOXED iframe inside mm-composer and is composited as a real layer (effects / masks / blend all apply) and survives bake. The SDK: `ctx` (2D on an OffscreenCanvas sized to the layer), `frame` (the `logicinputs.js` shape: pointer / touch / keyboard / scroll / gyro / audio / dt / time, coords 0..1), `controls.get(key)` (schema defaults + any wired `param:` binding), `content[]` ({kind, bitmap} from the wired `in` port). Numeric `controls` auto-expose `param:<key>` ports so input-pointer / op-math / state-* still drive the sketch. Fetch `GET /__logic_guide?section=sketch` for the full contract + recipes. This is project-local (one file), self-contained, and needs NO daemon restart - prefer it to rung 3 for anything that is "one app needs a custom interaction".
...
3. **Extend the runtime engine (editor-global).** The interaction needs a CAPABILITY the runtime doesn't have AND it should become a REUSABLE primitive for every project (not a one-off) - a new effect/position mode, a new processor node, a new uniform. Edit the engine: the evaluator in `editor/tools/_shared/logicgraph.js`, the effect / position / render runtime in `editor/tools/mmcomposer/` (and its slimPlayer twin so baked output matches), or the kind's spec wiring in `editor/app.js`. This is a normal move when the capability is durable; for a one-off interaction prefer rung 1.5 (a sketch). Grep these to find where your primitive's capability lives; do not reverse-engineer blindly.

Climbing past rung 1 is expected, not exceptional. The `testnoodle` failure (a per-row camera time-delay declined because "the custom shader only gets the current frame") is now a **rung-1.5 job**: write a `sketch` that keeps its own frame ring-buffer and draws the per-row delay - no engine edit, instantly previewable. Declining, or jumping straight to a global engine edit for a one-off, is the bug.
