...
- If that entry carries a `motionPlate` block (art-director §4.7 - present only when a video provider was wired), it is the **temporal half of the brief, user-approved**: `motionPlate.observedMotion` binds the emotional + pacing register (drift speed, breathing, camera energy) the same way `inheritPaletteHexes` binds the palette - the research fleet commits its registers as a TRANSLATION of that observed motion, never an independent pick. Thread `motionPlate.path` into the research envelope as the motion reference.
