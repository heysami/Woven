...
**When sound-orchestrator ran first, it already made these decisions.** Check for a `pe_sound_<nxId>` node in the workflow before the `ambient` drawer commissions anything: it carries the committed register, the voice cast (with delivery direction), the narration scripts and their assetPaths, the music bed and the loudness plan. When it exists, the ambient drawer does NOT re-commission - it loads the listed `assetPath` files into its WebAudio graph and follows `outputs.voiceCast` and `outputs.loudness` verbatim, so the piece's sound matches the project's committed world instead of a second, parallel one. A plan-only run leaves the assetPaths null and the `synthFallback` descriptors populated, which is the zero-spend path. When no such node exists, the ambient drawer commissions its own audio exactly as described next.
...
- If that entry carries a `motionPlate` block (art-director §4.7 - present only when a video provider was wired and the user opted in), it is the **temporal half of the brief, user-approved**: `motionPlate.observed` (measured `energyBand`/`settleMs`/`loopPeriodS` + the frame-by-frame analysis) binds the emotional + pacing register the same way `inheritPaletteHexes` binds the palette - the research fleet commits its registers as a TRANSLATION of it, never an independent pick. `motionPlate.keyframes[]` are the approved references for motion states the still plate does not contain - thread them + `motionPlate.path` into the research envelope.
...
The recurring case: an `nx-mount` iframe carries a walkable 3D scene in the hero slot; the host page is a long-form document scrolling below. Inside the iframe, the runtime sets `touch-action: none; overscroll-behavior: none; user-select: none` on the canvas - drag-to-look owns the gesture. **This creates a recurring conflict** with the host page that wants behaviour of its own. Three failure modes:
...
**Rule B - host-level guaranteed scroll-down affordance.** The hand-off envelope tells the chat caller to ensure the host HTML wraps the iframe in a section that includes an absolutely-positioned `<a href="#next-section">` (or a button) with `pointer-events: auto` and `z-index` above the iframe - the user's "I want out" escape hatch on touch devices. An absolutely-positioned `.scene__enter`-style "Go deeper ↓" link is the canonical pattern. Without it, `touch-action: none` traps the user on mobile.

**Rule C - overlay pointer-events budget (text passes through, controls restore).** Every absolute-positioned chrome layer over the iframe defaults to `pointer-events: none` on its container, with `pointer-events: auto` restored only on real interactive children (buttons, links). The canonical pattern: `.scene__overlay { pointer-events: none; } .scene__overlay .scene__voice { pointer-events: auto; }`. The final QA+lens gate relies on this: the craft lens on the assembled runtime rejects an overlay container with blanket `pointer-events: auto` that covers the iframe.
...
- **`registerDirective`** - prose (no word arrays). Write the interpretive part of each drawer brief in the vocabulary dramaturgy / experience-design actually uses FOR THIS PIECE - the words fall out of the committed emotional register + pacing feel + paradigm, never a house word-bag. Derive them per project: a reverent slow-bath place briefs its moments as *thresholds* the visitor crosses and *dwells* the piece asks them to hold; a progressive-reveal portrait briefs its beats as *the turn* where attention shifts and the *reveal* it earns; a walkable memorial briefs presence as the *dwell* before the next *beat* (illustrative, non-binding - re-derive from this piece's own register, never paste this list). Only fire this register for the committed medium (paradigm) - a `2d-illustrative` scroll vignette and a `3d-environment` walk earn different words.
- **`principleStance`** - prose of what "real" means for THIS piece, phrased for holistic lens judgment the way `successFeel` is written, NOT a checklist: presence over feature (the visitor inhabits a place, they do not operate a widget); the committed pacing register is HELD (a slow-bath never rushes to the next beat, an immediate-immersion never stalls the arrival); the visitor leaves changed, per `successFeel` ("the room holds them", "they leave quieter"). A stance the concept lens weighs whole, not items it ticks off.
...
- **`presenceMechanics`** - per spine moment, WHICH mechanic makes it inhabited rather than paged (capabilities.py HARD CHECK E): the non-scroll input channel that answers the visitor there, the ambient life it holds at rest, and how its transition travels through space instead of crossfading. Written into `deliverables.json` as ledger items (format `presence` / `interactive-input`) so the final gate audits them with probes, not prose. A research pass that commits a scroll spine of plates with no presenceMechanics has committed a slideshow - re-derive or reroute to a website build.
- **`freedomMoments`** - WHICH spine moments release control to the visitor (HARD CHECK E point 4): where the piece stops steering and lets them explore embodied - walk / orbit / pan a scene wider than the frame / choose the order of attention - with the story WAITING and resuming only on their own signal (a threshold crossed, a turning away), never a timer. Cursor-parallax is the world answering, NOT embodiment - it cannot be the piece's only agency. Declare each freedom moment's bounds + resume signal; ledger items (format `embodiment`) let the gate probe that visitor input actually displaces the view/body and that the story does not advance uninvited mid-exploration. This is the difference between a guided tour and a place: even a reverent memorial gives the visitor somewhere to stand and the choice of when to move on.
- **`copyDosage`** - per spine moment, the METERED story delivery (capabilities.py HARD CHECK F): what is on stage (one beat = one idea, a line or two the visitor absorbs in one breath - never a pre-assembled title+kicker+body paragraph), the authored ORDER the channels enter (scene settles, then the line, then the detail/sound - real pauses sized for absorbing, not a 100ms stagger), the sub-beats longer material splits into and what earns each (scroll-dwell / time / attention), the authored REST before the next beat, and which beats (if any) are NAMED crescendos allowed to flood the senses. The spine drawer bakes this into its keyframe table; ledger items (format `pacing`) let the gate walk the runtime beat by beat and count copy clusters + on-stage words against the dosage. Copy that arrives as a wall - however beautifully graded - fails concept, because a flood is not a telling.
- **Degree of inhabitation** (for `3d-environment`) - scripted-flythrough / hybrid-with-held-zones / fully-walkable. **→ Render via the SHARED layer:** a `3d-environment` narrative no longer hand-builds `nx_scene_` in 3D; the orchestrator co-dispatches `scene-3d-orchestrator` with `mode: host-driven` and `drivenHandles` = whatever the `spine`/`reveals` move (the camera dolly, doors of light, artifacts that listen back), and the spine drives the place via `window.__scene3d.step(state, alpha)`. **A narrative place is an inhabitable world, so the co-dispatch passes `immersionHint: immersive-place`, a `locationArchetypeHint` picked from `docs/research/location-archetype-library.index.json` to match the brief's place (a studio → `indoor-architectural`, a reef → `underwater`, a memorial garden → `outdoor-natural`, a dream → `surreal-dreamscape`), and a `fidelityHint` derived from the aesthetic register (painterly/volumetric → a `stylized-<family>` from `immersive-world-study.md` §5; naturalistic → `photoreal`).** scene-3d's own research validates the archetype and commits the six-pillar block; the resulting scene is judged by the aesthetic-lens immersive realism block at the final gate. The `2d-illustrative` / `iconographic-anim` paradigms keep their own scene drawer. A recommendation only on inhabitation shape; scene-3d-orchestrator owns its own internal gating of the felt-shape (this family does not lens-gate it - it is a co-dispatched collaborator, like visual-orchestrator). **The resulting scene MUST satisfy the "3D must feel 3D" contract** from `capabilities.py` HARD CHECK D (enforced by the scene-3d lens gates): the user can look around AND move inside (WASD/touch joystick for walkable, click-to-fly between authored anchors for guided, or a pausable/scrubbable dolly for cinematic). Static locked-camera 3D fails this contract regardless of render quality - use `2d-illustrative` instead. Hero artefacts need interactive rotation, continuous self-motion, or three-dimensional light response - flat-lit meshes earn a craft-lens block. scene-3d's research commits the `renderSource` (three.js default; `three.js-webgpu`/TSL per `3D_CAPABILITIES.md` §1.4 when the PLACE is itself a material meditation, used sparingly; the Spline runtime when the curator supplies a `.splinecode`; Meshy glTF heroes when wired), a `texturePolicy` (painted plates for painterly registers, PBR for realistic - generated via visual-orchestrator co-dispatch), and an `effectsBudget` (atmosphere-first: fog, dust motes, water, cloth - contemplative, never juice for juice's sake) - and decomposes the place into subsystems, each rendered + verified standalone.
...
**Voice of the brief** - phrase the interpretive portion of each drawer envelope in `research.registerDirective` at `buildRegister.cadence`, deriving the actual words from this piece's committed emotional and pacing register, not a house vocabulary. State `research.principleStance` as the quality bar. Do NOT name a profession or write "imagine you are". Keep structural spec (files, primitives, lens gates, paths) literal. Fire the register only for the committed medium.
...
One hero-slot piece can surface many symptoms at once - the entrance won't scroll, items overlap, dragging inside the piece is dead, chrome buttons don't click, scroll moves a tiny distance then stalls - and one symptom often has two root causes. **Fix by root cause, never symptom-by-symptom** - what follows is the root-cause taxonomy, not a symptom catalogue.
...
**Hero-slot pieces (full-bleed first-viewport iframes) MUST NOT skip Phase F.** An assembled runtime can pass the final QA+lens gate on its own surface and still be broken at the iframe ↔ host boundary; only Phase F catches this.
