...
- `facing` - REQUIRED - the subject's orientation as the CAMERA sees it, in plain words (e.g. "seen from above, facing down toward the bottom of the frame" for top-down, "side profile facing right" for side-scroll). Most sprite failures are ART-side, not instruction-side: the plate is briefed one facing and GENERATED another, and every animation frame inherits the error. The base plate's generation prompt must state this facing verbatim, and the scaffolded `animated-sprite` node carries it as its `facing` field - the daemon then vision-verifies the plate BEFORE the sheet spend and the sheet's first frame before commit, hard-failing on mismatch (per-cycle override allowed when a cycle faces differently, e.g. `turn`)
...
- `loop` semantics (seamless-return vs play-once). A play-once cycle ALSO commits `endState` - what the final frame must show (e.g. "slumped flat on the ground" for `hit`) - carried onto the node so the daemon verifies the last frame before committing the sheet. Looping cycles need no end state (they return to the start pose)
