...
**Damped smoothing toward host-fed targets MUST carry a teleport guard.** If your handles accept authoritative transforms (`setXTransforms(arr)`-style) and your `onFrame` eases rendered positions toward them (critically-damped pursuit - correct, smoothness is the quality signal), you must SNAP instead of ease whenever the target jumps farther than any legitimate per-frame motion (`if (gapSq > SNAP_DIST²) cur = base` before the pursuit step; pick SNAP_DIST a few × the fastest per-frame move). Hosts recycle instance slots - a minion killed at the north tower and respawned at the south base is the SAME instance index with a target 80u away - and ambient wrap-around does the same. Without the guard the model visibly SLIDES across the whole scene to catch up, which reads as a glitching unit. A snap between two frames reads as despawn/respawn; a multi-frame slide through the battlefield never looks right.
