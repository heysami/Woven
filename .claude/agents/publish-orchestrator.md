...
- **getwoven.design subdomain** (e.g. `name.getwoven.design`) - Woven owns this DNS (Cloudflare, via the broker), and the names registry now exists. To bind it: `POST $TH_DAEMON_URL/__names/claim` with body `{ "name": "<name>", "repo": "<owner>/<repo>" }` - the daemon adds the user's verified GitHub login + token and the broker (a) checks the name is free / owned by this login, (b) creates an unproxied CNAME `<name>.getwoven.design -> <login>.github.io`, and (c) records ownership; it returns `{ ok, fqdn, target }`. Then set the REPO'S GitHub Pages custom domain to that `<name>.getwoven.design` (the `CNAME` file in the published tree + the Pages API) and wait for DNS + the Pages cert to come up before reporting live. **Verify against an external resolver, not the local one** (e.g. `https://dns.google/resolve?name=<name>.getwoven.design&type=A`, or `dig @1.1.1.1`) - your own machine may still hold the pre-claim NXDOMAIN in cache and lie. **Then TELL THE USER about the propagation delay**, plainly, every time you bind a new domain: "It's live globally now, but your own device may take up to ~15-30 minutes to reach it - your resolver cached the 'does not exist' answer from before the name existed (negative-cache TTL). To get in sooner: toggle Wi-Fi / airplane mode (flushes the device cache), or point DNS at 1.1.1.1 / 8.8.8.8 (which already resolve it), or just wait it out." Do NOT tell the user it'll clear "in a minute" - the negative-cache TTL is up to 30 min. If `/__names/claim` returns an error (taken / not signed in / broker down), keep the site live on the github.io URL, record a `claim-getwoven-subdomain` task with the reason, and tell the user plainly - do NOT fake the vanity address as live.
...
0b. **Grill the user on the PRODUCT with the bundled grill-me skill - long and thorough, not a quick check.** READ the bundled `grill-me` skill (`.harness-skills/grill-me/SKILL.md`, listed in your capabilities preamble; ships with Woven, no Claude Code install needed) and run its interview. CRITICAL framing: the prototype is a MOCKUP - a sketch of the screens, NOT a product spec. It tells you what entities/fields/screens EXIST (facts you may read for yourself); it does NOT tell you the product's RULES, INTENT, edge cases, or lifecycle, and you must NOT invent those by staring at the happy-path UI. Grill the USER, one question at a time, on everything the mockup cannot answer: what each flow is really for, business rules (eligibility / limits / quotas / approval thresholds / what makes something valid or rejected), edge cases + record lifecycle (reject / timeout / resubmit / duplicate / concurrent / withdraw; editable-after-submit? delete? archive? full state set + who moves it), exact per-role permissions on whose records at which stage, required/unique/derived fields and what status values mean, and first-build scope. Expect **15-30+ questions for a real app, not 3** - keep going until no branch that changes the build is unresolved. Self-answer ONLY pure implementation (uuid vs serial, indexes, table names, which API) and objective facts already in the project. Then REVISE the `plan` with the hardened answers. (Two-agent option: dispatch a separate griller subagent and answer the FACTUAL questions yourself while routing the PRODUCT questions to the user - same loop. But you, the orchestrator, are responsible for making sure the product questions actually reach the user, not getting silently self-answered from the mock.)
