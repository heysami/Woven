...
In the SAME pass as `research.md` + `inventory.json`, write `test-cases.json` NEXT TO them (`source/{branch}/scrapbooks/{sbId}/test-cases.json`). The QA gate (`GET /__qa/run`) auto-detects the file and walks EVERY case end to end instead of the generic battery: the plan defines its own proof. Schema + runner: `docs/features/qa-test-cases.md` and `editor/tools/qa/README.md` (self-test fixture: `editor/tools/qa/fixtures/cases-demo.test-cases.json`).

Scrapbook is a whole-page build with no intent funnel, so OMIT `harness` and `intents` (the runner skips its harness preflight when no intents / `intent` steps are present); cases drive the real page with raw pointer steps. Required contents:

- `phases`: every state the page can be in (initial, revealed, mid-scroll, post-rearrange). `phaseExpr`: the JS expression that reads the current one.
- `phaseSetups`: steps that drive the page INTO each phase (the abuse / soak reuse them).
- `cases` (journeys): the full scroll journey top to bottom and back, drag-rearrange per draggable cutout class, click-flip / tap-to-reveal per flip-bearing cutout class (per the committed `interactionPrimitive`), and restart-after-end where the pacing has a terminal state (e.g. `interactive-discovery` reveal). Assert via `eval` checks on element state plus `notBlank`/`frameChanged`. Plus any state x input combination you already see being fragile.
...
End your final hand-back message with the case summary (e.g. "test-cases: 4 journeys + 3 abuse + soak 30s") so the plan gate can surface it to the user.
