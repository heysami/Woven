...
```bash
cat "$TH_PROTOCOL_ROOT/.claude/agents/sim-text-art-scene-builder.md" \
  || cat "$TH_PROJECT_ROOT/.claude/agents/sim-text-art-scene-builder.md"
```
...
divergeAxis:     "glyph-register"
divergeValue:    "pure-ascii" | "unicode-box" | "unicode-block-shading"
=== END ENVELOPE ===
```
...
```js
let _lastFrameT = 0;
const FRAME_MS = 1000 / 18;   // 18 fps target
window.__scene = {
  onFrame(state, alpha) {
    const now = state.t;   // tick-time, not wall-time
    if (now - _lastFrameT < FRAME_MS) return;   // skip render this rAF
    _lastFrameT = now;
    drawAll(state, alpha);
  },
  fps,
...
A craft-lens dispatch will measure frame-cadence and reject a glyph grid drawing at 60fps.
...
Each register commits its representative charset table at the top of the file:

```js
// pure-ascii
const GLYPHS = { hbar: '-', vbar: '|', corner: '+', ... };

// unicode-box
const GLYPHS = { hbar: '─', vbar: '│', corner_tl: '╔', corner_tr: '╗', ... };

// unicode-block-shading
const GLYPHS = { full: '█', heavy: '▓', medium: '▒', light: '░',
...
<style>
  :root {
    --tui-bg: #0b0d10;             /* deep terminal bg */
    --tui-fg: #d6deeb;             /* foreground glyphs */
    --tui-accent: #82aaff;         /* highlighted state */
    --tui-warn: #ffcb6b;
    --tui-err:  #ff5370;
  }
  .sim-canvas {
...
<script type="module">
  ...
  // Charset per register (committed at top per divergeValue):
  const GLYPHS = { /* see §3.4 + §4 */ };
  ...
  function ensureGrid() {
    ...
    _grid = new Array(ROWS);
    for (let r = 0; r < ROWS; r++) _grid[r] = new Array(COLS).fill(' ');
  }
  ...
  function setCell(c, r, glyph, fg = COLORS.fg) {
    ...
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        ...
        if (typeof cell === 'string') {
          if (currentColor !== COLORS.fg) { ctx.fillStyle = COLORS.fg; currentColor = COLORS.fg; }
          ctx.fillText(cell, c * CELL_W, r * CELL_H);
        } else if (cell && cell.glyph) {
          if (currentColor !== cell.fg) { ctx.fillStyle = cell.fg; currentColor = cell.fg; }
          ctx.fillText(cell.glyph, c * CELL_W, r * CELL_H);
        }
      }
    }
  }
  ...
  window.__scene = {
    onFrame(state, alpha) {
      if (!_ready) return;          // fonts still loading
      const now = state.t;
      if (now - _lastFrameT < FRAME_MS && _lastFrameT > 0) return;
      _lastFrameT = now;
      ensureGrid();
