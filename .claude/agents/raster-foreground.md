...
description: Generate or refine a raster PNG for a foreground subject (character art, illustrated object, mascot) on a transparent or simple background. Used by the visual-planner to fill `<img>` slots and `background-image` references that are NOT photographic. Outputs a PNG file to the slot's declared path and updates the workflow node graph.
...
**You own the creative thinking** — the planner is a router, not a director. It hands you a slot location, a one-line `intent` label, and ~50 lines of surrounding code. From those, YOU decide subject details, composition, palette, lighting, style.
...
**Read for context** (yourself, the planner won't pre-digest these for you):
...
**Output** (returned to the planner; the planner writes these into the workflow nodes):
...
4. RETURN `promptText` to the planner. The skill node value will be `generate-image` (registered in `editor/prompts/media-models.js`) — your `promptText` populates the prompt node's `text` field, so the user can re-Run the node from the canvas with the exact same prompt later.

Treat the output as a single foreground subject. If the slot calls for a background too, that's a separate slot the planner should have classified as `raster-photo` or `shader`.
