...
- If that entry carries a `motionPlate` block (art-director §4.7 - present only when a video provider was wired), it is the **user-approved ambient idle in pixels**: `motionPlate.observedMotion` binds the idle energy band + how materials behave under motion (refraction shift, sway amplitude, drift speed) - `s3d-research-technique` commits the ambient idle spec + interaction easing constants as a translation of it, and it anchors the runtime composer's §8.7 ambient-energy axis. Thread `motionPlate.path` into the research envelope as the motion reference.
