...
```jsonc
{
  ...
  "qaChecklist": [
    ...
    "immersive-place ONLY: the caller passes immersionMode + locationArchetype + fidelityRegister to the aesthetic-lens so its immersive realism block runs (no dead shadows, nothing-bare, distance-holds, color-script vs archetype paletteHexes, always-moving)"
  ]
}
```
