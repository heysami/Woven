...
1. **Load the canonical fixture.** Read `source/<branch>/_coherence/model.json` (or wherever `cp_fixture` wrote it). This is the source of truth — every entity, every fact, declared once.
2. **Enumerate the pages.** Glob `source/<branch>/*.html` (typically `index.html`, `verdict-card.html`, `audit-trail.html` — three pages).
...
5. **Append findings** to `source/<branch>/COHERENCE_REPORT.json` (create if missing). Each finding has shape:
   ```json
...
1. **Load the chrome contract.** Read `source/<branch>/_coherence/chrome.contract.json` — has shape `{brandSymbolId, navItems, sealSelector, navLocation, ...}`.
