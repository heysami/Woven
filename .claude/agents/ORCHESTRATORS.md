...
To **add a new entry**: create a new `prototype/<prefix>-<entryId>.md` (copy any existing file as a template), then re-run `python3 scripts/build-library-indexes.py` to refresh the index. To **edit an entry**: open its file directly, change what you need, re-run the script.
...
Auto-generated by `scripts/build-library-indexes.py` (scans `prototype/<prefix>-*.md` files and builds the index from their frontmatter). The orchestrator's discovery layer — read-once-per-session, ~30–80 KB structured JSON, no prose-scanning.
...
```jsonc
{
  ...
  "entries": {
    "<entryId>": {
      ...
      "notForUseWhen":       "<one line>",
      "pairsPrototypes":     ["<prototypeSlug>", ...],
      "sourceFile":          "prototype/<prefix>-<entryId>.md"
    }
  }
}
```
...
6. Drawer reads index.entries[styleId].sourceFile (prototype/<prefix>-<styleId>.md, ~1-5 KB).
...
new prototype/<prefix>-<entryId>.md.
```
...
1. **Write the primer** at `docs/research/<name>-library.md`:
   ...
   - Universal negative-keyword list
   - Implementation / orchestrator-integration notes
2. **Write each per-entry source file** at `prototype/<prefix>-<entryId>.md`. Each is hand-edited, ~1-5 KB. Use any existing photo/illust/material entry as a template:
...
4. **Run the script**: `python3 scripts/build-library-indexes.py`. Verify it scanned the prototype/ files and built `≥1 decisionTree slug`.
...
7. **Write the per-slot / per-element drawer playbook** following `photography-style-enricher.md` / `material-fidelity-author.md` as templates. The drawer reads `prototype/<prefix>-<entryId>.md` directly (it's the source of truth — no library slicing).
...
To edit an existing entry: open `prototype/<prefix>-<entryId>.md` directly. Re-run `python3 scripts/build-library-indexes.py` after committing. The primer file is only edited when the FUNDAMENTALS change (e.g. you learned a new film stock worth naming, or a new universal anti-pattern).
