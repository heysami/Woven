...
Generated by `scripts/regen-prototype-details.py`. **Self-contained** — title + summary + the verbatim YAML block of that one entry + when-NOT-to-use + pairs-with + image placeholder. ~1-5 KB per file (~2 KB average for photo/illust, ~3-5 KB for material because of CSS/SVG/GLSL implementation snippets).

**These files do double duty** — same file is the human-browsable Design library card AND the orchestrator/drawer's runtime read at dispatch:

- **Human path**: System tab → Design library → click on `photo-helmut-newton-flash` card → see the full library entry without opening the 13K-word library.
- **Orchestrator path**: drawer reads `prototype/photo-<styleId>.md` directly. Self-contained YAML, no sed-slice needed. ~1-2K tokens per dispatch instead of paging the big library.

Drop reference images per entry at `prototype/<sample>.png`; the catalog UI surfaces them.
...
When a new library entry is added, regenerate via `python3 scripts/regen-prototype-details.py` then `python3 scripts/build-library-indexes.py`. The latter picks up the new lineRange for orchestrator fallback (when a per-entry file is missing or stale).
...
3. JSON-only filter on index.entries[styleId].roleAffinity + .antiPatternKeywords + .notForUseWhen.
   No library file or per-entry file touched yet.
4. Optional: index.entries[secondaryStyleId] for register chaining.
5. Pass picked styleId to the per-slot drawer.
6. Drawer reads prototype/<prefix>-<styleId>.md directly (~1-5 KB, self-contained).
7. Drawer extracts YAML fields (examplePromptTemplate, keywords, anti-patterns, implementation snippets) and composes the output.

Fallback if per-entry file is missing (freshly-added library entry not yet regenerated):
   6b. sed -n '<start>,<end>p' library.md using index.lineRange → same content, slightly more expensive.
   Surface the gap so the user re-runs scripts/regen-prototype-details.py.
```

Per-slot cost: ~3 KB of index (one-time per session) + ~1-5 KB of per-entry file per slot. A 20-slot page is ~30-100 KB total instead of 300+ KB of prose-scanning. The big library file is never read at dispatch time.
