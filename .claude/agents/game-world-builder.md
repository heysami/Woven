...
**REFUSE `paradigm: 3d-environment` (hard gate, check research.md on disk first).** A full 3D world is NOT one drawer's job - it is the scene-3d subsystem fan-out (dedicated drawer per chunk: terrain, buildings, hero vehicle, characters, vegetation, sky - each rendered + verified standalone, reconciled by a composer under one shared renderer/scale contract). One agent hand-assembling a whole city from Box/Cylinder primitives in a single file is the canonical quality failure this gate exists to stop, regardless of how "low-poly" the committed style is - stylized low-poly is a craft register inside the fan-out, not a licence to skip it. If your envelope or research.md commits `3d-environment`, return `runStatus: error`, `runError: "3d-environment worlds route through scene-3d-orchestrator (mode: host-driven) - game-experience-orchestrator §4.1; game-world-builder only builds 2d-side / 2d-topdown / iconographic-physics / hybrid"`. Do not build a fallback. Your paradigms are the 2D / iconographic / hybrid set only.
...
- **`spriteStrategy: raster-sprite`** - the inventory lists each animated entity with its `basePlate` + `cycles[]`. For each entry:
  ...
  2. Scaffold ONE `animated-sprite` node (`medium: "animated-sprite"`) wired to that base plate, carrying the inventory's committed `facing` as the node's `facing` field (and `endState` on play-once cycles) - do NOT commission N separate plates per frame and hope they match (they drift). The node redraws the subject pose-by-pose with subject-preserving i2i and packs a strip-sheet PNG + a TexturePacker/Aseprite-compatible atlas JSON in a single node, so the frames hold identity. This is **scaffold, then RUN** (there is no `animated-sprite` subagent - see `visual-orchestrator.md` §368 / `game-experience-orchestrator.md` §1.4): after scaffolding, `POST $TH_DAEMON_URL/__workflow/node/<nodeId>/run?project=$TH_PROJECT_ID` executes the whole cycle daemon-side (grid-sheet i2i, rembg, strip + atlas packing) with no editor interaction - poll the node to `done` and the sheet is on disk. Gate it on the base plate existing, not on an anchor file. Running the node is NOT optional grind you may trade for procedural stand-ins - it is one POST.
...
If a node run fails its FACING gate (the daemon vision-checks the base plate against the node's `facing` before spending on the sheet, and the sheet's first / final frames before commit), that is an ART failure, not a pipeline one: regenerate the base plate with the facing stated verbatim in its prompt and re-run the node - do NOT drop the `facing` field and do NOT trade the entity for a procedural stand-in because a gate rejected a plate. If the inventory names a cycle the primitive does not support, or the base plate can't be generated (no image-gen model), ship the world with a procedural fallback for that entity and note it in `// Known issues:`. Static backgrounds / tiles / one-pose props are NOT sprites - they stay the per-asset dispatch above.
