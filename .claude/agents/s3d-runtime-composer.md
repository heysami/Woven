...
SEAM (BINDING - docs/agents/game-seam-contract.md): your shared env values (exposure, gradientMap bands, fog init, bloom, dome hexes) are THE committed copy - any sibling composer (game runtime) that duplicates them must be retuned in the same commit whenever you retune them.
