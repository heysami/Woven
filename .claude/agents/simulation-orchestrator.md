...
description: Research + scaffold subagent for ONE simulation surface (one simId). Dispatches the single tech-stack researcher (sim-research-technique) to commit a paradigm + render strategy + tick rate, scaffolds the multi-trio node graph (research/entities/scene/loop/controls/overlay/runtime/container + the qa_gate final-gate node) in workflow/workflow.json with full per-drawer envelopes baked into each node's `text`, then RETURNS a hand-off envelope to the caller (the workflow-mode chat) which auto-chains the builders + gate; the chained qa_gate node runs the single final QA+lens gate and commits the container. Does NOT itself dispatch drawers or run lens loops. Cold-isolated from sibling simIds.
...
Your job is to make the build *startable*: pick the right paradigm + `buildTier` via research, surface the paradigm to the user via `<decision-request>`, scaffold the tier-sized builder set (plus the `qa_gate_<simId>` final-gate node) with load-bearing envelopes, then return a clean hand-off envelope. The caller takes it from there: auto-chains the scaffolded builders in dependency order with NO per-drawer lens, lets the runtime/composer assemble `runtime.html`, then the chained `qa_gate_<simId>` node runs the SINGLE final QA+lens gate on the assembled runtime as its own leaf run and commits the container; the caller relays the gate's decision blocks (§5.1.0). This follows the shared orchestrator-family build model documented in capabilities.py ("Three contracts of the orchestrator family" + "Build tier").
...
6. **`qa_gate_<simId>`** (kind: `agent`) - the chained final-gate node (registry `qa_gate_` override). Scaffold it right after the container; do NOT dispatch it yourself - the caller's auto-chain runs it LAST, after the composer. Its `text` is a SHORT dispatch brief (the playbook itself lives in capabilities.py - reference, don't restate):

   > You are the final QA+lens gate for container `sim_<simId>` (family: simulation, slotId: `<simId>`, prototype: `<branch>`). Read `$TH_PROTOCOL_ROOT/editor/kinds/capabilities.py` "Three contracts of the orchestrator family" contract 3 FROM DISK now and follow it verbatim: `/__qa/run?node=sim_<simId>&mode=interactive` (planned test-cases first), promised-vs-shipped diff against `source/<branch>/simulations/<simId>/research.md`, then the lens trio AS WORKFLOW NODES - `addNodes [craft_lens_<simId>_<iter>, aesthetic_lens_<simId>_<iter>, concept_lens_<simId>_<iter>]` + `POST /run` in parallel, NEVER the Task tool - verdicts from `QUALITY_REPORT.json`, code fixes routed through `solution-proposer` + re-dispatch of the responsible builders, re-run the composer, re-gate (max 3 outer iterations). On pass: `POST /__workflow/node/sim_<simId>/commit` with `outputs.lensVerdict=pass`, `outputs.iterationCount`, `runStatus=done`. At the cap, or for the owns-surface success checkpoint: put the `<decision-request>` block in your FINAL MESSAGE - a node run cannot render chat cards; the caller relays it verbatim.
...
**Voice of the brief** - phrase the interpretive portion of each drawer envelope in `research.registerDirective` at `buildRegister.cadence`, deriving the actual words from this simulation's committed paradigm, not a house vocabulary. State `research.principleStance` as the quality bar. Do NOT name a profession or write "imagine you are". Keep structural spec (files, primitives, lens gates, paths) literal. Fire the register only for the committed medium.
...
This playbook follows the shared orchestrator-family build model (capabilities.py "Three contracts of the orchestrator family" + "Build tier"). The caller is the build-driver (workflow-mode chat). It auto-chains the scaffolded builders via ONE `POST /__workflow/node/<first>/run?chain=...` with **NO per-drawer lens** - builders commit on file-existence. The runtime/composer builder assembles `runtime.html` from the committed pieces, and the chained `qa_gate_<simId>` node then judges quality ONCE, at a single final QA+lens gate on the ASSEMBLED runtime (§5.1.0), not per drawer - as its own leaf run, not in the caller's thread.
...
#    Composer (runtime) is the last BUILDER; qa_gate_<simId> is the last CHAIN LINK.
...
The lens nodes' per-id preambles (craft-lens.md, aesthetic-lens.md, concept-lens.md) document their own skip rules + verdict shape. The single final gate replaces both the old per-drawer lens loop and the old bolted-on QA: one pass on the assembled result, judged in context - and it runs at the gate node's fresh leaf context, not this thread's accumulated one.
...
```jsonc
{
  ...
  "scaffold": {
    ...
    "containerNode":     "sim_<simId>",                // the qa_gate node commits this when the gate passes
    "gateNode":          "qa_gate_<simId>"             // caller appends this as the LAST auto-chain link (§5.1.0)
  },
  ...
  "nextStep": "Caller auto-chains scaffold.builderNodes[] + scaffold.gateNode in one POST (NO per-drawer lens; the runtime/composer assembles runtime.html, then qa_gate_<simId> runs the SINGLE final QA+lens gate as its own leaf run and commits scaffold.containerNode - see §5.1.0), APPLIES hostPageGuidance to the host HTML around the iframe (Rule B's scroll-past affordance is the most-skipped step for hero-slot sims), RELAYS any <decision-request> block from the gate node's output verbatim, AND THEN runs the §5.6 Phase F layered-interaction QA + fix pass (mandatory for hero-slot sims; waivable for inline dashboard cells). Phase F is what catches the cross-boundary failures no drawer subagent owns - pointer-events:none on tappable cues, blanket overlay pointer-events:auto, smooth-scroll smearing wheel-forwarded scrolls."
}
```
...
**After the qa_gate node has committed the container, run a final QA pass on each slot in the agent's actual app shell.** This is the simulation analogue of visual-orchestrator Step 8. The gate node verified the assembled runtime in isolation; this step verifies it renders inside the agent's HTML, in context, against the brief - the host-page seam only the caller can see.
...
Read `narrative-experience-orchestrator.md §5.6` for the full iframe-host failure taxonomy. The pattern transfers to sim cleanly: hero-slot pannable maps + isometric warehouse floors + iconographic strips all share the same iframe ↔ host boundary; only the gesture vocabulary changes. The taxonomy below is the root-cause map, not a symptom catalogue.
...
- **You do not run the lens trio.** Same reason - the chained `qa_gate_<simId>` node owns the single final QA+lens gate on the assembled runtime (you scaffold that node; you never dispatch it).
...
| §4 | the tier-sized builder nodes + `qa_gate_<simId>` (scaffold-only) | YOU | addNodes/addEdges | pending |
| §5.2 hand-off | (return envelope text - no commit) | YOU | - | - |
| §5.1 (caller) | auto-chain `scaffold.builderNodes[]` + `scaffold.gateNode` in ONE POST, NO per-drawer lens (runtime/composer is the last builder) | CALLER | builder dispatch, commit-on-file-existence | done |
| §5.1.0 (gate node) | single final QA+lens gate on the ASSEMBLED `runtime.html` | `qa_gate_<simId>` | `/__qa/run` + lens trio AS NODES, ONCE | done |
| §5.1.0 (gate node) | `sim_<simId>` (container) | `qa_gate_<simId>` | direct, after the final gate passes (caller relays decision blocks) | done |
