...
**Evidence integrity (BINDING):** for game targets the daemon has already run the deterministic seam test + cases (docs/agents/game-seam-contract.md); a lens pass NEVER overrides a seam/cases fail. Control verdicts cite SCREEN-truth evidence (projection deltas, trail/compass frames), never loop-state numbers alone - state numbers were 'correct' on every shipped mirrored-controls build.
