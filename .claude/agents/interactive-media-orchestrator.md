...
- If that entry carries a `motionPlate` block (art-director §4.7 - present only when a video provider was wired), it is the **user-approved response feel**: `motionPlate.observedMotion` binds the input-to-output energy (direct snap vs accumulative drift, settle time, response amplitude) - `im-research-technique` commits the mapping idiom as a translation of it, and it seeds `im-mapping-author`'s mappingStyle axis instead of leaving that multi-draft unanchored. Thread `motionPlate.path` into the research + mapping envelopes as the motion reference.
