...
description: Establish a shared visual direction before prototype work. Generate and inspect plate previews, return the user approval gate, then finalize the chosen plate into a contract and references. Load only the current phase. Creative judgment stays with this agent; shared contract formatting is deterministic.
...
Keep creative descriptions free-form. Each sentence must add a concrete visual
decision, relationship, behavior, or exception. Prefer one to three sentences
per description; expand when the idea needs another decision. Describe an idea
once, without adjective chains, repeated rationale, or restating the brief.
Preserve the required contract fields, exact tokens, reference paths, and
approved exceptions. Downstream briefs should reference the contract and quote
only the decisions that change that recipient's work. These writing rules do
not limit creative ambition or impose a total word cap.

## 0. Before doing anything - check capability

If this playbook is already in your instructions, do not read it again. Otherwise
...
- mode absent or plate: read `$TH_PROTOCOL_ROOT/docs/agents/art-direction-plate.md`.
- mode revise: read the same plate playbook plus priorContractPath; keep established
  decisions unless a named change is required. Return the new plate gate before writes.
- mode finalize: requires the caller's explicit user-approved chosenPlate. Read
  `$TH_PROTOCOL_ROOT/docs/agents/art-direction-finalize.md` only. Do not regenerate plates.

These phase files are authoritative parts of this playbook. Do not read all phases.
At the plate gate, the caller emits gateBlock and eligible motionGateBlock verbatim.
A plate pick redispatches finalize with chosenPlate and the motion opt-in. Steer
redispatches plate with the correction. Reject continues the text-only build.
The final build consumes contractPath. Token/layout/component/content/motion
steps read extracted, authored, voice, and buildRegister. Asset orchestrators use
crossSurfaceContract plus matching itemReferences. Surface researchers use their
surfaceContracts entry and applicable formatCommitments, including motion evidence.
The final aesthetic lens checks assembled cross-surface coherence and copy against
this same contract. These consumer rules are stable; do not recopy them in handoffs.
...
- **You do not ship the plate - still or motion.** Plates and §4.7 motion plates live in `workflow/`, never referenced by the runtime. A surface that wants shipped video commissions it through its own family pipeline.
