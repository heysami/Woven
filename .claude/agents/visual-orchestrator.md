...
- `animated-sprite` slots are **scaffolded, not dispatched** - there is no `animated-sprite` `Task` subagent. For each `medium: "animated-sprite"` entry the parent adds an `animated-sprite` NODE wired to the slot's base image (a `raster-foreground` plate, generated first in wave1 when the sprite is character-linked); the node's `edit`-port authoring then redraws the subject pose-by-pose with subject-preserving i2i and packs the strip PNG + atlas JSON. No PNG lands at a single `outputPath`, so it is exempt from the wave2 anchor-file check - gate it on the base plate existing instead. Set the node's `facing` field (and `endState` for play-once cycles) from the slot's brief whenever the subject has a committed orientation: the daemon's headless run vision-verifies the base plate against `facing` BEFORE the sheet spend and the generated first/final frames before commit, because sprite failures are art-side (a plate briefed "facing down" generated facing right poisons every frame).
...
| video              | `video`            | `video-gen`       | A       | **fal Veo 3.1 / Luma Ray 2 / Kling / Pika / Hailuo wired in `media-models.js` v3.4.1 (real `.mp4` output)**. Requires `TH_FAL_API_KEY`. If no key → drawer STOPS and surfaces the limitation; degrade the slot to a generated still + CSS motion (do NOT re-route raster-substance frames to `motion` - HTML-faked photorealism reads uncanny; `motion` is for vector-substance frames on its own merits, not as a key fallback). |
...
For any slot that needs MOTION (i.e. wouldn't be served by a still image), decide with the SAME principle that already splits stills between `vector-mark` (Claude draws the SVG) and `raster-photo`/`raster-foreground` (a model generates the pixels): **the substance of the frame decides the medium.** Freeze the intended motion at any representative frame and classify that frame as if it were a still slot:

| Freeze a frame - what is its substance? | Pick |
|---|---|
| **Raster substance** - photographic light, continuous tone, texture, physical matter in motion: a real face talking, a hand opening a door, fabric, liquid, weather, product-in-the-world footage. The frame would classify as `raster-photo` / `raster-foreground`. | `video` (fal Veo 3.1 / Kling / etc.). Requires a video provider `✓ KEY` - if none is wired, do NOT reconceive the frame as HTML (faked photorealism reads cheap); degrade to a generated still + CSS motion, or surface the limitation. |
| **Vector substance** - type, flat shapes, graphic illustration, diagrams, UI-narrative, multi-clip authored scenes: typography reveals, shape transforms, animated section intros, product-tour beats. The frame would classify as `vector-mark` / `vector-icon`. Anything you'd build in After Effects from shapes and text rather than shoot. | `motion` (Hyperframes). No key involved - and a wired video key is NEVER a reason to promote a drawn frame to diffusion footage, exactly as an image key is never a reason to rasterise a logo. |
...
This is a PER-SLOT judgment, not a per-project one: a photographic page can carry one kinetic-type moment (`motion`), and a vector-native page can hold one filmic window (`video`). Do not inherit the medium from the project's overall imagery register - classify each slot's own frame. **Mixed frame** (filmic footage UNDER crisp headline/UI): the substance split IS the layer split - the footage layer is `video`, the type stays real DOM on top (quiet-zone composition); never put load-bearing text inside a diffusion frame.

Default for a `<video>` HTML tag with no existing source file: classify its intended frame per the table - `video` for raster substance, `motion` for vector substance. Key presence gates only whether `video` CAN run; it never argues for `video` over `motion`, and it never argues against `motion` (which needs no key, plays in-browser, and can be rendered to a real video later via Hyperframes). Two symmetric failure modes to avoid: resolving drawn frames to `video` because a key is wired (garbled type, off-brand palette, diffusion sheen on a graphic register), and resolving filmic frames to `motion` out of caution (uncanny HTML-faked realism - the bias that once kept the video lane at zero use). The generation cost gate already ran at the orchestrator plan gate, so don't re-litigate cost per slot in either direction.
