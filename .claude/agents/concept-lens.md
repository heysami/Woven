...
| **Paradigm fit** | Does the rendered output match `prdParadigm`? If the PRD said `2d-spatial-map` and the runtime is a bar chart - paradigm violation. If `3d-environment` and the scene is a flat top-down - violation. Binary pass/fail, but counted as a block-failure on fail. **You may NOT infer a committed mechanism from how the frame looks.** A hybrid/route-table paradigm ("stations 2/3/9 are genuine host-driven 3d") is verified against the MACHINERY, not the pixels: read the slot's `deliverables.json` / research route table from disk and eval the runtime's registry (e.g. is a live 3d module actually mounted for that station, or is it the 2d/poster fallback?). A well-crafted 2d substitute LOOKS like a "3d-threshold moment" in a screenshot - crediting it as "delivered as promised" is exactly how arty's three missing 3d stations passed this lens. If a committed route is substituted, paradigmFit = fail (block), even when the substitute feels excellent - note the feel in prose, fail the score. | `paradigmFit` |
