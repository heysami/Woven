...
**Check for committed sound direction FIRST.** If sound-orchestrator ran on this project there is a `pe_sound_<gameId>` node in the workflow carrying the whole audio decision, and it OVERRIDES your own register improvisation:

```bash
curl -fsS "$TH_DAEMON_URL/__workflow?project=$TH_PROJECT_ID" \
  | python3 -c "import json,sys; d=json.load(sys.stdin); print(json.dumps([n for n in d.get('nodes',[]) if n.get('id')=='pe_sound_<gameId>'], indent=2))"
```

When it exists, take `outputs.loudness` as your gain budget verbatim (do not re-derive it from the register table), decode the clips at `outputs.sfxPalette[].assetPath` for the events they name, and use each entry's `synthFallback` recipe for anything parametric-per-hit or missing. `outputs.musicDirection.assetPath` is your bed. A plan-only run leaves every `assetPath` null and the `synthFallback` descriptors populated: that is the zero-spend path, and it still gives you a coherent register to synthesise into. When the node does not exist, behave exactly as described below.
