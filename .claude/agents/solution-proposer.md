...
6. **Target the JUDGED artefact, and every duplicated copy.** The gate failed the ASSEMBLED runtime - a fix that lands only in a sibling artefact the gate never renders (the scene3d standalone runtime, a subsystem selftest harness, a spec template) resolves nothing. When the root cause lives in a value that is DUPLICATED across composers (a game runtime that re-authors the scene3d composer's shared renderer env: `toneMappingExposure`, cel `gradientMap` bands, fog init, bloom params), the fixPlan must name EVERY file carrying the constant and say "port to both in lockstep" - grep the sibling composer before writing the plan. (The named trap: palvalley's cel-ramp/exposure fix was applied to `scene3d/.../runtime.html` only; the judged `games/.../runtime.html` kept its stale copies and the fail shipped while the report said RESOLVED.)
