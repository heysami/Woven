...
**When `artContract` is non-null** it is authoritative over the bare aesthetic-slug default: every composed prompt MUST hit `artContract.crossSurfaceContract.imageryRegister`, so the illustrations land in the same world as the chrome. The decision-tree lookup below still selects the library styleId; bias toward the candidate matching the contract's register and bake the contract's material read into `materialHint`. **For `colorHint`, give the palette CHARACTER (the register + the contract's `colorUsePrinciple` - e.g. "concentrated accent punches on a quiet ground"), NOT the full `sharedPaletteHexes`.** Do NOT force every illustration to carry all the brand colours - the per-slot colour SUBSET + weight (small→accent, large→dominant/neutral) is assigned later by visual-orchestrator's step 3.5, which knows each slot's size and position; it appends the concrete subset to the prompt at scaffold time. When `artContract` carries `buildRegister`, phrase your style/line/material/role enrichment briefs in that register (derived per project for the illustrative medium, never a fixed word list, never user copy) so the project's craft-language stays coherent. If `artContract` is null, behave exactly as before.
