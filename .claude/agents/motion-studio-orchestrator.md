...
- If that entry carries a `motionPlate` block (art-director §4.7 - present only when a video provider was wired), it is the **user-approved transition register**: `motionPlate.observedMotion` binds the cinematic pacing band (cut energy, drift speed, camera behaviour) - `ms-research-technique` commits the transition register as a translation of it. Additionally, `motionPlate.path` is a LEGAL i2v/motion reference for this family: thread it to `ms-concept-frames-author` and `ms-scene-composer` alongside the still plate, so per-scene assets inherit the approved motion character, not just the approved look.
