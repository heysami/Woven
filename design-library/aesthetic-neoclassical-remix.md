---
# Sample-image references for the System-tab design library. Images sit
# next to this file in design-library/ and were generated via image_gen.
images:
  - src: aesthetic-neoclassical-remix-ui.png
    reason: Aesthetic vocabulary in UI.
  - src: aesthetic-neoclassical-remix-isolated.png
    reason: Signature motif, isolated.
---
# Neoclassical Remix (aesthetic)

**Tag:** Antiquity-meets-now (Squarespace "Browser History" renaissance campaign; Gucci Ignasi Monreal SS18; Daniel Arsham eroded classics; museum-merch culture)

**Canonical references:**
- Squarespace *Browser History* — real person photographed among renaissance statues, modern device in hand.
- Gucci SS18 (Ignasi Monreal) — old-master paintings hosting modern product.
- Daniel Arsham — eroded classical busts as contemporary sculpture.
- Versace/medusa lineage; museum-gift-shop poster culture.

## Cultural identity

Classical antiquity (statues, oil paintings, columns, engravings) staged WITH contemporary elements — a phone in a painted hand, a tracksuit beside a marble torso, UI chrome over an engraved sky. The collision claims timelessness for the new thing: "this product belongs in the canon." Distinct from `aesthetic-vaporwave` (which uses the bust ironically, as a dead-mall artifact) — Neoclassical Remix is sincere and art-directed, museum-grade reproduction quality with ONE precise anachronism.

The defining gesture is **the canonical artifact + the modern intrusion**, both rendered at full fidelity, composed like a museum plate.

**Palette anchor:** marble cream `#efe9dd`, stone gray `#b9b3a7`, oil-painting umber `#5c4630` + Naples gold `#c8a96e`, gallery-wall white or deep gallery green `#1f312a`; ink near-black `#171513`; one modern accent (Yves-Klein blue `#1f2fd4` or signal red) for the contemporary layer.

## Motifs / imagery vocabulary

- ONE classical anchor per screen: statue, painted figure, column fragment, engraved diagram — photographed/painted straight, not duotone-memed.
- ONE modern intrusion: device, garment, product, UI window — placed with curatorial precision.
- Museum apparatus: plaque-style captions ("Marble, ca. 130 AD — and iPhone"), catalog numbers, serif small-caps.
- Engraving/etching line-work as decorative rules and section dividers.
- Generous gallery whitespace; content hung like works on a wall.

**Raster required:** the statue/painting photography or painted-style render (photo `surreal-tableau` composed mode, illust `classical-painting-remix`). CSS cannot produce the canon.

## Voice register

Curatorial wit: plaque-formal sentences that land a modern word ("Oil on canvas. Ships in 48 hours."). Latin permitted once. Never meme-speak over the artworks.

## Failure mode

Duotone bust + vaporwave grid = wrong lineage (that's vaporwave). Five statues per viewport = clipart museum. The discipline: full-fidelity artifact, one anachronism, museum spacing, plaque captions.

## Best for

Brand campaigns claiming heritage, portfolio/agency sites with wit, fashion drops, museum and auction-house digital, editorial features on tech-meets-tradition.

## Pairs well with

- **Shells:** `shell-centered-column` (gallery hang), `shell-editorial-broken-grid`, `shell-horizontal-scroll-stage` (collection walk).
- **Styles:** `style-serif-warm-paper`, `style-editorial-italic-accent`, `style-restrained-hairline` for the modern layer.
- **Photo/Illust:** photo `surreal-tableau`; illust `classical-painting-remix` (the painted variant).
- **Aesthetic kin:** `aesthetic-vaporwave` (ironic cousin), `aesthetic-dark-academia` (scholarly cousin), `aesthetic-maximalism`.
