...
- Scroll-fade between flavor sections; product rotates or swaps per section.

**Raster required:** the product photography (clean cutout, photo `apple-clean-studio` register on pastel). The illustrations are flat vector (illust `jean-jullien-thick-line`, `spectrums-organic-blob` registers).

## Voice register

Pantry wit: short, punny, benefit-honest. "Aromatic confidence." "Toothpaste, minus the tube." Lowercase permitted; exclamation marks rationed to one per page.

## Failure mode

Gradient pastels (flat is the conviction); five motifs per viewport (sparse or it's `curly-girly`); gray corporate ink (the deep ink must be a FLAVOR, not #333); product on white instead of on the pastel (kills the shelf).

## Best for

DTC food & beverage, personal care and supplements, pet brands, playful pharma/wellness, any product line where each SKU owns a color.

## Pairs well with

- **Shells:** `shell-hero-stack` (flavor per section), `shell-horizontal-scroll-stage` (shelf walk), `shell-centered-column`.
- **Styles:** `style-flat-design` (host), `style-bold-display`, `style-editorial-italic-accent` for premium variants.
- **Aesthetic kin:** `aesthetic-curly-girly` (denser cousin), `aesthetic-monochrome-pop-poster` (loud cousin), `aesthetic-positivity-kawaii` (mascot cousin).
- **Photo:** `apple-clean-studio` on pastel grounds; food photography registers.
