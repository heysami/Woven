...
**Palette anchor:** sky `#dfeefc` → `#fdf2e3` (dawn) or `#eef6ff` (day), meadow `#7fb069` / `#9dc183`, soil `#8a6f4d`, UI paper `#ffffff` at 92-100% opacity, ink `#22303c`, one bloom accent (poppy `#ff6b5e`, lavender `#9a8cff`, or sun `#ffc24d`).
...
**Raster required:** the landscape (photo `eco-infrastructure` family, or illustration `3d-dreamscape-terrain` / `ghibli-watercolor-bg` for painterly briefs). CSS gradients alone produce "default Tailwind hero", not pastoral.

## Voice register

Exhale-length sentences. "Work smarter. Move faster." cadence is permitted but better is quieter: "Your week, in one calm view." Never urgency, never FOMO, never exclamation marks.

## Failure mode

A stock meadow photo at 100% saturation behind 9 feature cards + bouncy scroll animations + three accent colors = travel-brochure SaaS. The discipline: ONE landscape, low horizon, muted bloom accent, UI that stays out of the sky.

## Best for

Productivity and focus tools, HR/wellbeing platforms, healthcare and therapy products, agritech, sustainability dashboards, financial planning with a "long-term peace" positioning.

## Pairs well with

- **Shells:** `shell-hero-stack`, `shell-centered-column`, `shell-scroll-journey-scene` (walk through the land), `shell-two-column-app` (app briefs).
- **Styles:** `style-restrained-hairline`, `style-editorial-italic-accent`, `style-cream-humanist` for editorial variants.
- **Aesthetic kin:** `aesthetic-organic-overgrowth` (surreal sibling), `aesthetic-solarpunk` (civic-futurist sibling).
- **Photo/Illust:** photo `eco-infrastructure`, illust `3d-dreamscape-terrain`, illust `lofi-contemplative` (dusk register), `ghibli-watercolor-bg`.
