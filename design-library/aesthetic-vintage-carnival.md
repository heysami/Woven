...
**Raster required:** woodcut illustrations + aged-paper texture + halftone photography (style `raster-cutout` handling; material `letterpress-emboss`, `uncoated-paper`). Clean vector ornaments read as craft-beer template, not carnival.
...
Food & drink brands with provenance stories, festivals and events, breweries/wineries/roasters, barbershops and makers, album and tour pages, anything selling craft + theater.

## Pairs well with

- **Shells:** `shell-centered-column` (the poster), `shell-hero-stack`, `shell-horizontal-scroll-stage` (showbill per panel).
- **Styles:** `style-serif-warm-paper`, `style-raster-cutout`; type via illust `typo-wood-type-letterpress`.
- **Materials:** `letterpress-emboss`, `uncoated-paper`, `halftone-cmyk`.
- **Aesthetic kin:** `aesthetic-steampunk` (fantasy cousin), `aesthetic-dieselpunk` (interwar cousin).
